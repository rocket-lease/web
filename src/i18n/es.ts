export const es = {  // --- App ---
  'app.name': 'Rocket Lease',
  'app.tagline': 'Alquila el vehiculo que necesitas',
  'app.role.rentadorActive': 'Rentando',
  'app.role.conductorActive': 'Conduciendo',
  'app.role.switchToRentador': 'Cambiar a modo rentador',
  'app.role.switchToConductor': 'Cambiar a modo conductor',

  // --- Nav ---
  'nav.buscar': 'Buscar',
  'nav.favoritos': 'Favoritos',
  'nav.reservas': 'Reservas',
  'nav.notificaciones': 'Notificaciones',
  'nav.soporte': 'Soporte',
  'nav.perfil': 'Perfil',
  'nav.dashboard': 'Dashboard',
  'nav.misVehiculos': 'Mis vehiculos',
  'nav.misReservas': 'Mis reservas',
  'nav.mapa': 'Mapa',
  'nav.historial': 'Historial',

  // --- Wallet / saldo ---
  'wallet.title': 'Saldo y retiros',
  'wallet.shortcut': 'Retirá tu saldo disponible o mirá los movimientos',
  'wallet.balanceLabel': 'Saldo disponible',
  'wallet.balanceHint': 'El saldo se acredita automáticamente cuando una reserva finaliza.',
  'wallet.emptyAccounts.title': 'Necesitás una cuenta bancaria',
  'wallet.emptyAccounts.description': 'Registrá una cuenta bancaria para retirar tu saldo disponible.',
  'wallet.emptyAccounts.cta': 'Ir a cuentas bancarias',
  'wallet.withdraw.title': 'Solicitar retiro',
  'wallet.withdraw.subtitle': 'Elegí una cuenta bancaria registrada y pedí la transferencia.',
  'wallet.withdraw.bankAccount': 'Cuenta bancaria destino',
  'wallet.withdraw.amount': 'Monto a retirar',
  'wallet.withdraw.amountHint': 'Ingresá el monto en pesos argentinos.',
  'wallet.withdraw.submit': 'Retirar saldo',
  'wallet.withdraw.processingHint': 'Cuando el retiro se procesa correctamente, queda en estado Procesada y luego depende de tu banco.',
  'wallet.withdraw.success': 'Retiro procesado. Verás la transaccion en tu banco.',
  'wallet.transactions.title': 'Movimientos',
  'wallet.transactions.subtitle': 'Acreditaciones por reservas y retiros realizados.',
  'wallet.transactions.empty': 'Todavía no tenés movimientos.',
  'wallet.transactions.credit': 'Acreditación por reserva',
  'wallet.transactions.debit': 'Retiro de saldo',
  'wallet.transactions.dispute_penalty_debit': 'Penalización por disputa',
  'wallet.transactions.dispute_penalty_credit': 'Compensación por disputa',
  'wallet.transactions.ticket_resolution_debit': 'Penalización por resolución de ticket',
  'wallet.transactions.ticket_resolution_credit': 'Compensación por resolución de ticket',
  'wallet.transactions.balanceAfter': 'Saldo luego del movimiento',

  // --- Mapa de rentadoras ---
  'mapa.title': 'Mapa de rentadoras',
  'mapa.nearMe': 'Cerca de mi',
  'mapa.radius': 'Radio',
  'mapa.locating': 'Obteniendo tu ubicacion...',
  'mapa.gpsDenied': 'Permiso de ubicacion denegado. Activalo para ver rentadoras cercanas.',
  'mapa.gpsUnavailable': 'Tu dispositivo no permite geolocalizacion.',
  'mapa.noKey': 'El mapa no esta disponible: falta configurar la API key de Google Maps.',
  'mapa.empty': 'No hay rentadoras disponibles en esta zona.',
  'mapa.loading': 'Cargando mapa...',
  'mapa.error': 'No se pudieron cargar las rentadoras.',
  'mapa.filters': 'Filtros',
  'mapa.filter.transmission': 'Transmision',
  'mapa.filter.transmissionAny': 'Cualquiera',
  'mapa.filter.maxPrice': 'Precio max. por dia',
  'mapa.filter.accessible': 'Accesible',
  'mapa.marker.vehicles': 'autos',
  'mapa.card.availableCars': 'autos disponibles',
  'mapa.card.priceFrom': 'Precio desde',
  'mapa.card.reputation': 'Reputacion',
  'mapa.card.viewVehicles': 'Ver vehiculos',
  'mapa.zone.tapToZoom': 'Toca para acercar',
  'mapa.zone.rentadoras': 'rentadoras',
  'mapa.googleKeyMissing': 'Falta configurar VITE_GOOGLE_MAPS_API_KEY para mostrar el mapa.',

  // --- Selector de ubicacion (rentador) ---
  'location.picker.label': 'Ubicacion del vehiculo',
  'location.picker.search': 'Escribi la direccion completa',
  'location.picker.searching': 'Buscando...',
  'location.picker.hint': 'Busca la direccion y ajusta el pin si hace falta.',
  'location.picker.approximate': 'Ubicacion aproximada — ajusta el pin.',
  'location.picker.noKey': 'Configura la API key de Google Maps para elegir la ubicacion.',
  'location.picker.selected': 'Ubicacion seleccionada',

  // --- Auth ---
  'auth.login.title': 'Bienvenido',
  'auth.login.subtitle': 'Ingresa a tu cuenta',
  'auth.login.email': 'Correo electronico',
  'auth.login.password': 'Contraseña',
  'auth.login.submit': 'Ingresar',
  'auth.login.noAccount': 'No tenes cuenta?',
  'auth.login.register': 'Registrate',
  'auth.login.forgot': 'Olvidaste tu contraseña?',
  'auth.login.error': 'Correo o contraseña incorrectos',
  'auth.login.errorUnverified': 'Verifica tu correo para poder ingresar',
  'auth.register.title': 'Crear cuenta',
  'auth.register.subtitle': 'Empeza a alquilar hoy',
  'auth.register.name': 'Nombre completo',
  'auth.register.email': 'Correo electronico',
  'auth.register.password': 'Contraseña',
  'auth.register.dni': 'DNI',
  'auth.register.phone': 'Telefono',
  'auth.register.confirmPassword': 'Confirmar contraseña',
  'auth.register.submit': 'Crear cuenta',
  'auth.register.hasAccount': 'Ya tenes cuenta?',
  'auth.register.login': 'Ingresa',
  'auth.register.passwordHint': 'Mínimo 8 caracteres, al menos una letra y un número.',
  'auth.register.pendingVerification': 'Ya existe una cuenta con este correo pendiente de verificación.',
  'auth.register.pendingVerificationCta': 'Reenviar código de verificación',
  'auth.register.pendingVerificationSending': 'Reenviando...',
  'auth.register.emailTaken': 'Ya existe una cuenta registrada con este correo.',
  'auth.recover.title': 'Recuperar contraseña',
  'auth.recover.subtitle': 'Te enviamos un enlace a tu correo',
  'auth.recover.email': 'Correo electronico',
  'auth.recover.submit': 'Enviar enlace',
  'auth.recover.back': 'Volver al inicio',
  'auth.recover.success': 'Revisa tu correo para recuperar tu contraseña',
  'auth.recover.newTitle': 'Nueva contraseña',
  'auth.recover.newSubtitle': 'Elegi una contraseña segura para tu cuenta',
  'auth.recover.newPassword': 'Nueva contraseña',
  'auth.recover.confirmPassword': 'Confirmar contraseña',
  'auth.recover.newSubmit': 'Guardar contraseña',
  'auth.recover.newSuccess': 'Contraseña actualizada. Ingresa con la nueva.',
  'auth.recover.linkExpired': 'El enlace expiro o ya fue usado. Pedi uno nuevo.',
  'auth.recover.mismatch': 'Las contraseñas no coinciden',
  'auth.verify.title': 'Verifica tu cuenta',
  'auth.verify.subtitleEmail': 'Te enviamos un codigo de 6 digitos a tu correo',
  'auth.verify.subtitlePhone': 'Ingresa el codigo para verificar tu telefono',
  'auth.verify.emailSection': 'Correo electronico',
  'auth.verify.phoneSection': 'Telefono',
  'auth.verify.code': 'Codigo',
  'auth.verify.submit': 'Verificar',
  'auth.verify.verifying': 'Verificando...',
  'auth.verify.sending': 'Enviando...',
  'auth.verify.resend': 'Reenviar codigo',
  'auth.verify.resendIn': 'Reenviar en {seconds}s',
  'auth.verify.sent': 'Codigo enviado',
  'auth.verify.success': 'Verificado correctamente',
  'auth.verify.verified': 'Verificado',
  'auth.verify.error.incorrect': 'Codigo incorrecto',
  'auth.verify.error.format': 'El código debe ser de 6 dígitos numéricos',
  'auth.verify.error.maxAttempts': 'Alcanzaste el máximo de intentos. Solicitá un nuevo código.',
  'auth.verify.banner': 'Tu cuenta no esta verificada',
  'auth.verify.banner.cta': 'Verificar',

  // --- Buscar ---
  'buscar.title': 'Buscar vehiculos',
  'buscar.placeholder': 'Marca o modelo...',
  'buscar.filters': 'Filtros',
  'buscar.filtersTitle': 'Filtrar vehiculos',
  'buscar.results': 'resultados',
  'buscar.noResults': 'No encontramos vehiculos con esos criterios',
  'buscar.loading': 'Buscando vehiculos...',
  'buscar.filter.transmission': 'Transmision',
  'buscar.filter.transmission.all': 'Cualquiera',
  'buscar.filter.transmission.automatic': 'Automatico',
  'buscar.filter.transmission.manual': 'Manual',
  'buscar.filter.transmission.clear': 'Limpiar filtro de transmision',
  'buscar.filter.priceRange': 'Precio por dia',
  'buscar.filter.accessibility': 'Accesibilidad',
  'buscar.filter.apply': 'Aplicar filtros',
  'buscar.filter.clear': 'Limpiar filtros',
  'buscar.noResultsHint': 'Probá con otra búsqueda o ajustá los filtros',
  'buscar.noResultsDatesHint': 'Probá ampliar el rango de fechas',
  'buscar.filter.dates': 'Fechas de alquiler',
  'buscar.filter.trunk': 'Baúl mínimo (litros)',
  'buscar.error': 'Error al cargar los vehículos',
  'buscar.location.city': 'Buenos Aires',
  'buscar.featuredIn': 'Destacados cerca tuyo',
  'buscar.placeholder.search':        'Marca, modelo...',
  'buscar.sort.priceAsc':             'Precio ↑',
  'buscar.sort.priceDesc':            'Precio ↓',
  'buscar.sort.rating':               'Mejor calificación',
  'buscar.sort.distance':             'Distancia',
  'buscar.sort.soon':                 'Próximamente',
  'buscar.filter.title':              'Filtros',
  'buscar.filter.sortBy':             'Ordenar por',
  'buscar.filter.minPrice':           'Precio mínimo',
  'buscar.filter.maxPrice':           'Precio máximo',
  'buscar.filter.seats':              'Asientos mínimos',
  'buscar.filter.year':               'Año',
  'buscar.filter.yearFrom':           'Año desde',
  'buscar.filter.yearTo':             'Año hasta',
  'buscar.filter.model':              'Modelo',
  'buscar.filter.accessible':         'Solo vehículos accesibles',
  'buscar.filter.characteristics':    'Características',
  'buscar.filter.clearAll':           'Limpiar todo',
  'buscar.filter.applyFilters':       'Aplicar',
  'buscar.ciudad':                    'Ciudad',
  'buscar.fechaDesde':                'Desde',
  'buscar.fechaHasta':                'Hasta',
  'buscar.cargando':                  'Buscando vehículos...',

  // --- Vehiculo ---
  'vehiculo.reservar': 'Reservar',
  'vehiculo.disponible': 'Disponible',
  'vehiculo.noDisponible': 'No disponible',
  'vehiculo.automatic': 'Automatico',
  'vehiculo.manual': 'Manual',
  'vehiculo.fuel.nafta': 'Nafta',
  'vehiculo.fuel.diesel': 'Diesel',
  'vehiculo.fuel.gnc': 'GNC',
  'vehiculo.fuel.electrico': 'Electrico',
  'vehiculo.fuel.hibrido': 'Hibrido',
  'vehiculo.seats': 'asientos',
  'vehiculo.perDay': '/ dia',
  'vehiculo.dynamicPricingFor': 'por {days} dias',
  'vehiculo.perWeek': '/ semana',
  'vehiculo.perMonth': '/ mes',
  'vehiculo.owner': 'Rentador',
  'vehiculo.rating': 'calificacion',
  'vehiculo.reviews': 'resenas',
  'vehiculo.reviewsTitle': 'Reseñas',
  'vehiculo.reviewsCount': '{count} reseñas',
  'vehiculo.reviewsEmpty': 'Este vehículo todavía no tiene reseñas.',
  'vehiculo.reviewsEmptyHint': 'Cuando alguien lo alquile y deje una opinión, vas a verla acá.',
  'vehiculo.unavailableNotice': 'Este vehículo no está disponible en este momento.',
  'vehiculo.ownNotice': 'No podés reservar tu propio vehículo.',
  'vehiculo.esMio': 'Es tu vehículo',
  'vehiculo.label': 'Vehículo',
  'vehiculo.description': 'Descripcion',
  'vehiculo.features': 'Caracteristicas',
  'vehiculo.rules': 'Condiciones',
  'vehiculo.rulesActive': 'Set de reglas activo',
  'vehiculo.rulesEmptyTitle': 'Sin restricciones configuradas',
  'vehiculo.rulesEmptyDesc': 'Este vehículo todavía no tiene un set de reglas asignado.',
  'vehiculo.rulesUnavailableTitle': 'Restricciones no disponibles',
  'vehiculo.rulesUnavailableDesc': 'El vehículo tiene un set asignado, pero no pudimos cargar sus condiciones.',
  'vehiculo.location': 'Ubicacion',

  // --- Caracteristicas ---
  'caracteristicas.gps': 'GPS',
  'caracteristicas.babySeat': 'Silla para bebe',
  'caracteristicas.sunroof': 'Techo solar',
  'caracteristicas.petFriendly': 'Apto para mascotas',
  'caracteristicas.wifi': 'WiFi',
  'caracteristicas.usbCharger': 'Cargador USB',
  'caracteristicas.auxCable': 'Cable AUX',
  'caracteristicas.bluetooth': 'Bluetooth',

  // --- Reservas ---
  'reservas.title': 'Mis reservas',
  'reservas.view.activas': 'Activas',
  'reservas.view.historial': 'Historial',
  'reservas.list.pendingExtension': 'Extensión pendiente',
  'reservas.list.extensionBadge': 'Extensión',
  'reservas.role.aria': 'Cambiar perspectiva',
  'reservas.role.conductor': 'Como conductor',
  'reservas.role.owner': 'Como rentador',
  'reservas.tabs.todas': 'Todas',
  'reservas.tabs.solicitudes': 'Solicitudes',
  'reservas.tabs.enRevision': 'En revisión',
  'reservas.tabs.pendientes': 'Pendientes',
  'reservas.tabs.pendientesSaldo': 'Pendientes de saldo',
  'reservas.tabs.confirmadas': 'Confirmadas',
  'reservas.tabs.enCurso': 'En curso',
  'reservas.tabs.completadas': 'Completadas',
  'reservas.tabs.canceladas': 'Canceladas',
  'reservas.filtros.placeholder': 'Filtrar por fecha',
  'reservas.filtros.title': 'Elegí un rango',
  'reservas.error': 'No pudimos cargar tus reservas. Intentá de nuevo.',
  'reservas.cargando': 'Cargando reservas',
  'reservas.paginacion.anterior': 'Anterior',
  'reservas.paginacion.siguiente': 'Siguiente',
  'reservas.badge.solicitudes': 'Solicitudes',
  'reservas.badge.enRevision': 'En revisión',
  'reservas.detail.idPrefix': 'Reserva #',
  'reservas.detail.contract.title': 'Contrato digital',
  'reservas.detail.contract.acceptedAt': 'Aceptado el {date}',
  'reservas.detail.actions.reportar': 'Reportar problema',
  'reservas.detail.cancel.cta': 'Cancelar reserva',
  'reservas.detail.cancel.modalTitle': '¿Cancelar la reserva?',
  'reservas.detail.cancel.modalBody':
    'No vas a poder recuperarla después. Si querés volver atrás más tarde, vas a tener que crear una nueva.',
  'reservas.detail.cancel.confirm': 'Cancelar reserva',
  'reservas.detail.cancel.canceling': 'Cancelando...',
  'reservas.detail.cancel.back': 'Volver',
  'reservas.detail.cancel.success': 'Reserva cancelada',
  'reservas.detail.cancel.refund.success': 'Recibiste {amount} RocketTokens de reembolso.',
  'reservas.detail.cancel.refund.preview': 'Sí, vas a recibir {amount} RocketTokens de reembolso.',
  'reservas.detail.cancel.noRefund.preview': 'No, esta cancelación no genera reembolso.',
  'reservas.detail.cancel.refund.unknown': 'No pudimos calcular un reembolso para esta reserva.',
  'reservas.detail.cancel.postPayUnavailable':
    'Las reservas confirmadas todavía no se pueden cancelar desde la app. Contactá a soporte si necesitás hacerlo.',
  'reservas.detail.cancellation.title': 'Reembolso por cancelación',
  'reservas.detail.cancellation.policy': 'Política aplicada',
  'reservas.detail.cancellation.rulesTitle': 'Reglas de la reserva',
  'reservas.detail.cancellation.policy.flexible': 'Flexible',
  'reservas.detail.cancellation.policy.moderate': 'Moderada',
  'reservas.detail.cancellation.policy.strict': 'Estricta',
  'reservas.detail.cancellation.policy.unknown': 'Sin política definida',
  'reservas.detail.cancellation.refund.flexible.active':
    'Recibirás un reembolso total si cancelás antes del {deadline}.',
  'reservas.detail.cancellation.refund.flexible.expired':
    'Ya no aplica reembolso para esta reserva según la política flexible.',
  'reservas.detail.cancellation.refund.moderate.active':
    'Recibirás un reembolso del 50% si cancelás antes del {deadline}.',
  'reservas.detail.cancellation.refund.moderate.expired':
    'Ya no aplica reembolso para esta reserva según la política moderada.',
  'reservas.detail.cancellation.refund.strict.active':
    'Podrás cancelar con reembolso hasta el {deadline}, siempre que todavía falten más de 48 horas para el inicio.',
  'reservas.detail.cancellation.refund.strict.expired':
    'Esta reserva ya no tiene reembolso disponible.',
  'reservas.detail.cancellation.refund.invalidDates':
    'No pudimos calcular el reembolso por fechas inválidas en la reserva.',
  'reservas.detail.cancellation.refund.missingPolicy':
    'Esta reserva no tiene una política de cancelación configurada.',
  'reservas.detail.cancellation.rules.policy': 'Política: {value}',
  'reservas.detail.cancellation.rules.deposit': 'Seña: {value}',
  'reservas.detail.cancellation.rules.kilometrage': 'Kilometraje: {value}',
  'reservas.detail.cancellation.rules.rentalTime': 'Duración del alquiler: {value}',
  'reservas.detail.cancellation.rules.missing':
    'No hay reglas de reserva disponibles para este vehículo.',
  'reservas.detail.cancellation.deposit.none': 'Sin seña',
  'reservas.detail.cancellation.deposit.ten': '10% de seña',
  'reservas.detail.cancellation.deposit.fifty': '50% de seña',
  'reservas.detail.cancellation.kilometrage.unlimited': 'Ilimitado',
  'reservas.detail.cancellation.kilometrage.limited': 'Hasta {value} km',
  'reservas.detail.cancellation.rentalTime.min': 'Mínimo {min} días',
  'reservas.detail.cancellation.rentalTime.max': 'Máximo {max} días',
  'reservas.detail.cancellation.rentalTime.between':
    'Entre {min} y {max} días',
  'reservas.empty.conductor.sinReservas': 'No tenés reservas en esta categoría todavía.',
  'reservas.empty.conductor.cta': 'Buscar vehículos',
  'reservas.empty.owner.sinReservas': 'No tenés reservas en esta categoría todavía.',
  'reservas.empty.owner.sinVehiculos': 'Publicá un vehículo para empezar a recibir reservas.',
  'reservas.empty.owner.publicarCta': 'Publicar vehículo',
  'reservas.estado.pending_payment': 'Pago pendiente',
  'reservas.estado.pending_balance': 'Señada',
  'reservas.estado.confirmed': 'Confirmada',
  'reservas.estado.in_progress': 'En curso',
  'reservas.estado.completed': 'Completada',
  'reservas.estado.cancelled': 'Cancelada',
  'reservas.estado.rejected': 'Rechazada',
  'reservas.estado.expired': 'Expirada',
  'reservas.detail.title': 'Detalle de reserva',
  'reservas.detail.voucher': 'Voucher QR',
  'reservas.detail.voucherHelp': 'Mostrá este código al rentador para retirar el vehículo.',
  'reservas.detail.cancel': 'Cancelar reserva',
  'reservas.detail.dispute': 'Reportar problema',
  'reservas.detail.dates': 'Fechas',
  'reservas.detail.pickup': 'Retiro',
  'reservas.detail.return': 'Devolucion',
  'reservas.detail.total': 'Total',
  'reservas.detail.breakdown.title': 'Desglose del precio',
  'reservas.detail.breakdown.original': 'Alquiler original',
  'reservas.detail.breakdown.extension': 'Extensión {n}',
  'reservas.detail.breakdown.totalSavings': 'Ahorro total',
  'reservas.detail.rentador': 'Rentador',
  'reservas.detalle.conductor': 'Conductor',
  'reservas.detalle.pago': 'Pago',
  'reservas.voucher.verifyTitle': 'Verificación de Voucher',
  'reservas.voucher.validTitle': 'Voucher Válido',
  'reservas.voucher.validBody': 'Esta reserva está confirmada y el vehículo puede ser entregado.',
  'reservas.voucher.invalidTitle': 'Voucher Inválido',
  'reservas.voucher.invalidBody': 'No pudimos encontrar una reserva válida con este código.',
  'reservas.voucher.statusWas': 'El estado actual es:',
  'reservas.voucher.details': 'Detalles de la Reserva',
  // --- Retiro / Devolución QR ---
  'reservas.retiro.cta': 'Entregar vehículo',
  'reservas.retiro.scannerTitle': 'Escaneá el QR del conductor',
  'reservas.retiro.fallback': 'Ingresar código manualmente',
  'reservas.retiro.confirmar': 'Confirmar entrega',
  'reservas.retiro.confirmando': 'Confirmando...',
  'reservas.retiro.exito': '¡Vehículo entregado! El alquiler comenzó.',
  'reservas.retiro.qrInvalido': 'Código inválido. Verificá con el conductor.',
  'reservas.retiro.dataMismatch': 'Los datos no coinciden',
  'reservas.retiro.dataMismatchToast': 'Contactá a soporte para reportar el problema.',
  'reservas.devolucion.cta': 'Devolver vehículo',
  'reservas.devolucion.scannerTitle': 'Escaneá el QR del rentador',
  'reservas.devolucion.confirmar': 'Confirmar devolución',
  'reservas.devolucion.confirmando': 'Confirmando...',
  'reservas.devolucion.exito': '¡Devolución registrada! El alquiler finalizó.',
  'reservas.devolucion.qrInvalido': 'Código inválido. Verificá con el rentador.',
  'reservas.devolucion.qrTitle': 'Tu código de devolución',
  'reservas.devolucion.qrHelp': 'Mostrá este QR al conductor para cerrar el alquiler.',
  'reservas.qr.cancelar': 'Cancelar',
  'reservas.qr.codigoPlaceholder': 'Pegá el código aquí',
  'conductor.reservas.solicitudesEnRevision': 'Solicitudes en revisión',
  'conductor.reservas.solicitudesEnRevisionHint': 'El rentador tiene 24 horas para responder.',
  'conductor.reservas.venceEnHoras': 'Vence en {count}h',
  'conductor.reservas.venceEnMenosDe1Hora': 'Vence en menos de 1 hora',
  'conductor.reservas.venceVencido': 'Solicitud vencida',
  'conductor.reservas.retirar.cta': 'Retirar solicitud',
  'conductor.reservas.retirar.modalTitle': '¿Retirar tu solicitud?',
  'conductor.reservas.retirar.modalBody': 'El rentador no la verá. Podés enviarla de nuevo más tarde.',
  'conductor.reservas.retirar.confirmar': 'Retirar',
  'conductor.reservas.retirar.cancelar': 'Volver',
  'conductor.reservas.retirar.retirando': 'Retirando...',
  'conductor.reservas.retirar.success': 'Solicitud retirada',
  'conductor.reservas.rechazoTitulo': 'Solicitud rechazada',
  'conductor.reservas.rechazoGenerico': 'El rentador no aceptó la solicitud',
  'conductor.reservas.tituloEnRevision': 'Esperando respuesta del rentador',

  // --- Rentador / Panel de reservas ---
  'rentador.reservas.title': 'Mis reservas',
  'rentador.reservas.tabs.todas': 'Todas',
  'rentador.reservas.tabs.solicitudes': 'Solicitudes',
  'rentador.reservas.tabs.pendientes': 'Pendientes',
  'rentador.reservas.tabs.confirmadas': 'Confirmadas',
  'rentador.reservas.tabs.enCurso': 'En curso',
  'rentador.reservas.tabs.completadas': 'Completadas',
  'rentador.reservas.tabs.canceladas': 'Canceladas',
  'rentador.reservas.filtros.desde': 'Desde',
  'rentador.reservas.filtros.hasta': 'Hasta',
  'rentador.reservas.filtros.limpiar': 'Limpiar filtros',
  'rentador.reservas.filtros.placeholder': 'Filtrar por fecha',
  'rentador.reservas.filtros.title': 'Elegí un rango',
  'rentador.reservas.empty.sinReservas': 'No tenes reservas en esta categoria todavia.',
  'rentador.reservas.empty.sinVehiculos': 'Publica un vehiculo para empezar a recibir reservas.',
  'rentador.reservas.empty.publicarCta': 'Publicar vehículo',
  'rentador.reservas.error': 'No pudimos cargar tus reservas. Intenta de nuevo.',
  'rentador.reservas.cargando': 'Cargando reservas',
  'rentador.reservas.paginacion.anterior': 'Anterior',
  'rentador.reservas.paginacion.siguiente': 'Siguiente',
  'rentador.reservas.detalle.title': 'Detalle de reserva',
  'rentador.reservas.detalle.conductor': 'Conductor',
  'rentador.reservas.detalle.fechas': 'Fechas',
  'rentador.reservas.detalle.desde': 'Desde',
  'rentador.reservas.detalle.hasta': 'Hasta',
  'rentador.reservas.detalle.total': 'Total',
  'rentador.reservas.detalle.pago': 'Pago',
  'rentador.reservas.detalle.metodo.credit_card': 'Tarjeta de crédito',
  'rentador.reservas.detalle.metodo.debit_card': 'Tarjeta de débito',
  'rentador.reservas.detalle.metodo.bank_transfer': 'Transferencia',
  'rentador.reservas.detalle.holdActivo': 'Esperando pago del conductor',
  'rentador.reservas.detalle.holdExpirado': 'Hold expirado',
  'rentador.reservas.detalle.noEncontrada': 'No encontramos esta reserva en tu panel.',
  'rentador.reservas.badge.solicitudes': 'Solicitudes',
  'rentador.reservas.detalle.aprobar': 'Aprobar',
  'rentador.reservas.detalle.rechazar': 'Rechazar',
  'rentador.reservas.detalle.aprobando': 'Aprobando...',
  'rentador.reservas.detalle.rechazando': 'Rechazando...',
  'rentador.reservas.detalle.solicitudInfo': 'Esta solicitud está esperando tu decisión.',
  'rentador.reservas.detalle.rejectionReason.titulo': 'Motivo del rechazo',
  'rentador.reservas.rechazar.modalTitle': 'Rechazar solicitud',
  'rentador.reservas.rechazar.razonLabel': 'Razón (opcional)',
  'rentador.reservas.rechazar.razonPlaceholder': 'Por ejemplo: vehículo en mantenimiento',
  'rentador.reservas.rechazar.cancelar': 'Cancelar',
  'rentador.reservas.rechazar.confirmar': 'Rechazar',
  'rentador.reservas.rechazar.charCounter': '{count}/280',
  'rentador.reservas.aprobar.confirmTitle': 'Aprobar solicitud',
  'rentador.reservas.aprobar.confirmBody': 'El conductor recibirá 10 minutos para pagar. Las solicitudes solapadas se van a rechazar automáticamente.',
  'rentador.reservas.aprobar.confirmar': 'Aprobar',
  'rentador.reservas.errorAccion': 'No pudimos completar la acción. Intentá de nuevo.',
  'rentador.reservas.aprobada': 'Solicitud aprobada',
  'rentador.reservas.rechazada': 'Solicitud rechazada',
  'rentador.reservas.extension.aprobada': 'Extensión aprobada',
  'rentador.reservas.extension.rechazada': 'Extensión rechazada',
  'reservas.estado.pending_approval': 'En revisión',
  'reservas.detail.pagar': 'Pagar',
  'reservas.pago.title': 'Pagar reserva',
  'reservas.pago.summary': 'Resumen',
  'reservas.pago.selectMethod': 'Seleccioná un método de pago',
  'reservas.pago.addMethod': 'Agregar un medio de pago',
  'reservas.pago.confirm': 'Confirmar pago',
  'reservas.pago.metodo.credit_card': 'Tarjeta de crédito',
  'reservas.pago.metodo.debit_card': 'Tarjeta de débito',
  'reservas.pago.metodo.bank_transfer': 'Transferencia bancaria',
  'reservas.pago.metodo.digital_wallet': 'Billetera virtual',
  'reservas.pago.billetera.mercadopago': 'Mercado Pago',
  'reservas.pago.billetera.uala': 'Ualá',
  'reservas.pago.selectWallet': 'Seleccioná tu billetera',
  'reservas.pago.transferInfo': 'Transferencia bancaria',
  'reservas.pago.transferHint': 'Vas a recibir los datos para transferir. La reserva queda pendiente hasta que se acredite.',
  'reserva.breakdown.depositNotice': 'Este alquiler requiere una seña del {percentage}% ({amount}) según las condiciones del rentador.',
  // --- Seña + saldo (US-26 / US-30) ---
  'reserva.breakdown.depositOption': 'Pagar seña ({percentage}% — {amount})',
  'reserva.breakdown.depositOptionHint': 'Asegurás la reserva ahora y pagás el saldo de {amount} antes del retiro.',
  'reserva.breakdown.fullOption': 'Pagar total ({amount})',
  'reserva.breakdown.fullOptionHint': 'Pagás todo ahora y la reserva queda confirmada.',
  'reserva.breakdown.chooseMode': '¿Cómo querés pagar?',
  'reservas.saldo.title': 'Pagar saldo',
  'reservas.saldo.cta': 'Pagar saldo',
  'reservas.saldo.depositPaid': 'Seña pagada',
  'reservas.saldo.remaining': 'Saldo pendiente',
  'reservas.saldo.dueAt': 'Vence el {date}',
  'reservas.saldo.alert24h': 'Tu saldo vence en menos de 24 horas. Completá el pago para no perder la reserva.',
  'reservas.saldo.confirm': 'Confirmar pago del saldo',
  'reservas.saldo.success': 'Saldo pagado. Tu reserva está confirmada.',
  'reservas.deposito.success': 'Seña confirmada. Tenés hasta {date} para pagar el saldo.',
  'reservas.transfer.title': 'Transferencia bancaria',
  'reservas.transfer.pending': 'Pendiente de acreditación',
  'reservas.transfer.expiresIn': 'Vence en',
  'reservas.transfer.details': 'Datos para transferir',
  'reservas.transfer.instructions': 'Instrucciones',
  'reservas.transfer.step1': 'Ingresá a tu home banking o app de tu banco/billetera.',
  'reservas.transfer.step2': 'Usá los datos de CBU/CVU o alias para realizar la transferencia por el monto exacto.',
  'reservas.transfer.step3': 'Cuando se acredite, la reserva se confirma automáticamente.',
  'reservas.transfer.done': 'Ya transferí, volver a reserva',

  // --- Perfil ---
  'perfil.title': 'Mi perfil',
  'perfil.editProfile': 'Editar perfil',
  'perfil.verification': 'Verificación',
  'perfil.preferences': 'Mis preferencias',
  'perfil.verified': 'Verificado',
  'perfil.pendingVerification': 'Verificacion pendiente',
  'perfil.identity.title': 'Verificación de identidad',
  'perfil.identity.pending': 'Verificación en proceso. Te avisaremos apenas quede aprobada.',
  'perfil.identity.retry': 'Reintentar verificación',
  'perfil.identity.cta': 'Verificar identidad',
  'perfil.identity.rejectedReason': 'Motivo del rechazo',
  'perfil.driverLicense.title': 'Verificación de licencia',
  'perfil.driverLicense.pending': 'Tu licencia está en revisión. Apenas se apruebe vas a poder reservar.',
  'perfil.driverLicense.retry': 'Reintentar licencia',
  'perfil.driverLicense.cta': 'Verificar licencia',
  'perfil.driverLicense.rejectedReason': 'Motivo del rechazo',
  'perfil.level': 'Nivel',
  'perfil.reputation': 'Reputacion',
  'perfil.publishedVehicles': 'Vehículos publicados',
  'perfil.reviewsTitle': 'Reseñas',
  'perfil.viewAsRentador': 'Ver como rentador',
  'perfil.viewAsConductor': 'Ver como conductor',
  'perfil.verif.identidad': 'Identidad verificada',
  'perfil.verif.licencia': 'Licencia verificada',
  'perfil.reviews.tabRentador': 'Como rentador',
  'perfil.reviews.tabConductor': 'Como conductor',
  'perfil.settings': 'Configuracion',
  'perfil.group.cuenta': 'Cuenta',
  'perfil.group.actividad': 'Actividad',
  'perfil.group.app': 'App',
  'perfil.adminPanel': 'Panel de administracion',
  'perfil.admin.row': 'Panel de administración',
  'perfil.darkMode': 'Modo oscuro',
  'perfil.notifications': 'Notificaciones',

  // --- Push notifications ---
  'push.title': 'Notificaciones',
  'push.subtitle': 'Recibí alertas de reservas y mensajes aunque no tengas la app abierta.',
  'push.active': 'Notificaciones activas',
  'push.active.hint': 'Te avisamos cuando tengas novedades sobre tus reservas o mensajes nuevos.',
  'push.enable': 'Activar notificaciones',
  'push.enableShort': 'Activar',
  'push.promptTitle': 'Activá las notificaciones',
  'push.dismiss': 'Ahora no',
  'push.installCta': 'Ver cómo',
  'push.manageDisable': 'Desactivar notificaciones',
  'push.disable': 'Desactivar',
  'push.denied': 'Permiso denegado',
  'push.denied.hint': 'Habilitá las notificaciones desde Configuración del sistema.',
  'push.unsupported': 'No disponible en este navegador',
  'push.needsInstall': 'Instalá la app primero',
  'push.needsInstall.hint': 'En iPhone, las notificaciones solo funcionan cuando agregás la app a tu pantalla de inicio.',
  'push.needsInstall.step1': 'Tocá el botón',
  'push.needsInstall.step2': 'Seleccioná "Agregar a pantalla de inicio"',
  'push.needsInstall.step3': 'Abrí la app desde el ícono y volvé aquí',
  'notificaciones.recent': 'Recientes',
  'notificaciones.empty': 'No tenés notificaciones',
  'notificaciones.emptyHint': 'Acá vas a ver las novedades de tus reservas y mensajes.',
  'notificaciones.error': 'No pudimos cargar tus notificaciones',
  'notificaciones.errorHint': 'Reintentá en unos segundos.',
  'notificaciones.unread': 'Sin leer',
  'notificaciones.markAllRead': 'Marcar todas como leídas',
  'notificaciones.markRead': 'Marcar como leída',
  'notificaciones.delete': 'Eliminar',
  'notificaciones.markedReadToast': 'Marcada como leída',
  'notificaciones.deletedToast': 'Notificación eliminada',
  'notificaciones.pushSettings': 'Notificaciones push',
  'notificaciones.pushSettingsHint': 'Recibí estas novedades en tu dispositivo aunque no tengas la app abierta.',
  'perfil.language': 'Idioma',
  'perfil.deleteAccount': 'Eliminar cuenta',
  'perfil.deleteAccount.title': 'Eliminar tu cuenta?',
  'perfil.deleteAccount.warning': 'Esta accion es permanente y no se puede deshacer.',
  'perfil.deleteAccount.consequences': 'Vas a perder acceso al historial de reservas, vehiculos publicados, resenas y datos de tu perfil.',
  'perfil.deleteAccount.confirmPrompt': 'Escribi "{word}" para confirmar',
  'perfil.deleteAccount.confirm': 'Eliminar mi cuenta',
  'perfil.deleteAccount.deleting': 'Eliminando...',
  'perfil.deleteAccount.success': 'Cuenta eliminada.',
  'perfil.deleteAccount.hasVehicles': 'No podés eliminar tu cuenta mientras tengas vehículos publicados. Eliminá tus vehículos primero.',
  'perfil.deleteAccount.hasActiveReservations': 'Tenés reservas activas. Cancelalas antes de eliminar tu cuenta.',
  'perfil.logout': 'Cerrar sesión',
  'perfil.switchRole': 'Cambiar a modo Rentador',
  'perfil.switchRoleConductor': 'Cambiar a modo Conductor',
  'perfil.level.bronze': 'Bronce',
  'perfil.level.silver': 'Plata',
  'perfil.level.gold': 'Oro',
  'perfil.level.platinum': 'Platino',
  'perfil.datos.title': 'Datos personales',
  'perfil.datos.subtitle': 'Tu información de contacto y preferencias',
  'perfil.verificaciones.title': 'Verificaciones',
  'perfil.verificaciones.row': 'Verificación',
  'perfil.cuentas.row': 'Cuentas bancarias',
  'perfil.beneficios.title': 'Beneficios',
  'perfil.beneficios.creditos': 'RocketTokens',
  'perfil.beneficios.cupones': 'Cupones',
  'perfil.beneficios.lealtad': 'Lealtad y nivel',
  'perfil.creditos.title': 'RocketTokens',
  'perfil.creditos.howTitle': 'Cómo funcionan',
  'perfil.creditos.earn': 'Ganás RocketTokens',
  'perfil.creditos.earnHint': 'Por cada reserva completada, resena dejada o referido.',
  'perfil.creditos.spend': 'Usalos en tu próxima reserva',
  'perfil.creditos.spendHint': 'Aplicalos como descuento al momento de pagar.',
  'perfil.creditos.bonus': 'Bonus por nivel',
  'perfil.creditos.bonusHint': 'A mayor nivel de lealtad, más RocketTokens por reserva.',
  'perfil.cupones.title': 'Cupones',
  'perfil.cupones.empty': 'Sin cupones activos',
  'perfil.cupones.emptyHint': 'Los cupones que recibas aparecerán acá.',
  'perfil.lealtad.title': 'Lealtad y nivel',
  'perfil.lealtad.allLevels': 'Todos los niveles',
  'perfil.form.name': 'Nombre completo',
  'perfil.form.phone': 'Telefono',
  'perfil.form.email': 'Email',
  'perfil.form.avatarUrl': 'URL de foto de perfil',
  'perfil.form.transmissionPreference': 'Preferencia de transmision',
  'perfil.form.maxPriceDaily': 'Precio maximo diario preferido',
  'perfil.form.accessibility': 'Accesibilidad (separado por coma)',
  'perfil.balance.title': 'Rocketokens',
  'perfil.balance.subtitle': 'Saldo disponible',
  'perfil.saveSuccess': 'Perfil actualizado',
  'perfil.saving': 'Guardando...',
  'vehiculo.instantBook': 'Reserva inmediata',
  'perfil.autoAccept.label': 'Auto-aceptar reservas en todos mis vehículos',
  'perfil.autoAccept.descripcion': 'Las nuevas reservas se confirman automáticamente sin tu revisión. Podés sobrescribir esta config por vehículo.',

  // --- Identidad ---
  'identidad.title': 'Verificación de identidad',
  'identidad.status.not_started': 'Sin iniciar',
  'identidad.status.pending': 'Verificación en proceso',
  'identidad.status.verified': 'Identidad verificada',
  'identidad.status.rejected': 'Verificación rechazada',
  'identidad.form.title': 'Subí tu DNI y una selfie',
  'identidad.form.copy': 'Necesitamos estas imágenes para habilitar reservas y publicaciones con mayor seguridad.',
  'identidad.form.front': 'Frente de tu identificación',
  'identidad.form.back': 'Dorso de tu identificación',
  'identidad.form.selfie': 'Selfie',
  'identidad.form.frontHelper': 'Foto nítida del frente, completa y legible.',
  'identidad.form.backHelper': 'Foto nítida del dorso, completa y legible.',
  'identidad.form.selfieHelper': 'Sacate una selfie mirando a cámara.',
  'identidad.form.pickFile': 'Elegir archivo',
  'identidad.form.noFile': 'No seleccionaste ningún archivo',
  'identidad.form.fileHint': 'Aceptamos imágenes JPG, PNG o HEIC.',
  'identidad.form.noteTitle': 'La verificación puede tardar',
  'identidad.form.noteCopy': 'El proceso de verificación de identidad no es instantaneo.',
  'identidad.form.acceptedCopy': 'Al enviar la verificación aceptas que revisemos tus imágenes para validar tu identidad.',
  'identidad.form.submit': 'Enviar verificación',
  'identidad.form.retry': 'Reintentar verificación',
  'identidad.form.submitting': 'Enviando...',
  'identidad.form.exit': 'Salir',
  'identidad.guard.cta': 'Ir a verificar identidad',
  'identidad.guard.reserve.title': 'Tenés que verificar tu identidad antes de reservar',
  'identidad.guard.reserve.copy': 'Para poder reservar un vehículo necesitás completar la verificación de DNI primero.',
  'identidad.guard.publish.title': 'Tenés que verificar tu identidad antes de publicar',
  'identidad.guard.publish.copy': 'Para publicar un vehículo necesitás completar la verificación de DNI primero.',
  'identidad.toast.submitted': 'Verificación enviada',
  'identidad.verified.copy': 'Tu identidad ya quedó verificada. Ya podés reservar y publicar normalmente.',
  'identidad.verified.cta': 'Volver al perfil',
  'identidad.pending.title': 'Tu verificación está en proceso',
  'identidad.pending.copy': 'No hace falta que esperes en esta pantalla. Podés volver cuando quieras y el estado se va a actualizar solo.',
  'identidad.pending.exit': 'Volver al perfil',
  'identidad.rejected.copy': 'Hubo un problema con la verificación. Revisá el motivo y volvé a enviar las imágenes.',
  'identidad.rejected.reasonTitle': 'Motivo informado por el proveedor',

  // --- Licencia ---
  'licencia.title': 'Verificación de licencia',
  'licencia.status.not_started': 'Sin iniciar',
  'licencia.status.pending': 'Verificación en proceso',
  'licencia.status.verified': 'Licencia verificada',
  'licencia.status.rejected': 'Verificación rechazada',
  'licencia.form.title': 'Subí tu licencia de conducir',
  'licencia.form.copy': 'Necesitamos la foto de tu licencia y una selfie para habilitar reservas.',
  'licencia.form.front': 'Frente de la licencia',
  'licencia.form.selfie': 'Selfie',
  'licencia.form.frontHelper': 'Foto nítida del frente, completa y legible.',
  'licencia.form.selfieHelper': 'Sacate una selfie mirando a cámara.',
  'licencia.form.pickFile': 'Elegir archivo',
  'licencia.form.noFile': 'No seleccionaste ningún archivo',
  'licencia.form.fileHint': 'Aceptamos imágenes JPG, PNG o HEIC.',
  'licencia.form.noteTitle': 'La verificación puede tardar',
  'licencia.form.noteCopy': 'El proceso de verificación de licencia no es instantaneo.',
  'licencia.form.acceptedCopy': 'Al enviar la verificación aceptas que revisemos tus imágenes para validar tu licencia.',
  'licencia.form.submit': 'Enviar licencia',
  'licencia.form.retry': 'Reintentar licencia',
  'licencia.form.submitting': 'Enviando...',
  'licencia.form.exit': 'Salir',
  'licencia.toast.submitted': 'Licencia enviada',
  'licencia.verified.copy': 'Tu licencia ya quedó verificada. Ya podés reservar normalmente.',
  'licencia.verified.cta': 'Volver al perfil',
  'licencia.pending.title': 'Tu licencia está en proceso',
  'licencia.pending.copy': 'No hace falta que esperes en esta pantalla. Podés volver cuando quieras y el estado se va a actualizar solo.',
  'licencia.pending.exit': 'Volver al perfil',
  'licencia.rejected.copy': 'Hubo un problema con la verificación. Revisá el motivo y volvé a enviar la licencia.',
  'licencia.rejected.reasonTitle': 'Motivo informado por el proveedor',

  // --- Verificaciones ---
  'verificaciones.gate.badge': 'Verificaciones pendientes',
  'verificaciones.gate.reserve.title': 'Necesitás completar tus verificaciones',
  'verificaciones.gate.reserve.copy': 'Para reservar un vehículo tenés que tener identidad y licencia de conducir verificadas.',
  'verificaciones.gate.publish.title': 'Necesitás completar tu identidad',
  'verificaciones.gate.publish.copy': 'Para publicar un vehículo tenés que verificar tu identidad primero.',
  'verificaciones.gate.cta': 'Continuar con la verificación',
  'verificaciones.status.not_started': 'Sin iniciar',
  'verificaciones.status.pending': 'En revisión',
  'verificaciones.status.verified': 'Completada',
  'verificaciones.status.rejected': 'Rechazada',
  'verificaciones.identity.title': 'Identidad',
  'verificaciones.identity.copy': 'Subí tu DNI para validar tu identidad.',
  'verificaciones.identity.cta': 'Verificar identidad',
  'verificaciones.identity.retry': 'Reintentar identidad',
  'verificaciones.driverLicense.title': 'Licencia de conducir',
  'verificaciones.driverLicense.copy': 'Subí tu licencia y una selfie para habilitar reservas.',
  'verificaciones.driverLicense.cta': 'Verificar licencia',
  'verificaciones.driverLicense.retry': 'Reintentar licencia',
  'verificaciones.rejected.reasonTitle': 'Motivo informado por el proveedor',

  // --- Bank accounts ---
  'bankAccount.title': 'Cuentas bancarias',
  'bankAccount.add': 'Nueva',
  'bankAccount.empty': 'No tenés cuentas bancarias vinculadas.',
  'bankAccount.identifierName': 'Nombre identificador',
  'bankAccount.cbu': 'CBU / CVU',
  'bankAccount.modal.title': 'Agregar cuenta bancaria',
  'bankAccount.modal.description': 'Completá los datos para vincular tu cuenta y poder publicar vehículos.',
  'bankAccount.form.identifierPlaceholder': 'Cuenta principal',
  'bankAccount.form.cbuPlaceholder': '1234567890123456789012',
  'bankAccount.form.identifierLengthError': 'Usá un nombre entre 3 y 50 caracteres para identificar esta cuenta.',
  'bankAccount.form.cbuError': 'El CBU o CVU debe tener exactamente 22 números.',
  'bankAccount.form.identifierRequired': 'Ingresá un nombre identificador.',
  'bankAccount.form.cbuRequired': 'Ingresá el CBU o CVU de la cuenta.',
  'bankAccount.form.submitError': 'No pudimos guardar la cuenta. Revisá los datos e intentá de nuevo.',
  'bankAccount.back': 'Volver',
  'bankAccount.deleteConfirm': '¿Eliminar cuenta bancaria?',
  'bankAccount.deleteConfirmPrompt': 'Esta acción es irreversible.',
  'bankAccount.emptyState.title': 'No tenés cuentas asociadas',
  'bankAccount.emptyState.description': 'Agregá una cuenta bancaria para poder recibir pagos de tus reservas.',
  'bankAccount.lastDeleted.title': 'Te quedaste sin cuentas bancarias',
  'bankAccount.lastDeleted.description': 'No vas a poder recibir pagos hasta agregar una nueva cuenta.',
  'bankAccount.detail.title': 'Detalle de cuenta',
  'bankAccount.detail.fullCbu': 'CBU completo',
  'bankAccount.detail.identifier': 'Nombre identificador',
  'bankAccount.goToAccounts': 'Ir a cuentas bancarias',
  'bankAccount.addSuccess': 'Cuenta agregada correctamente.',
  'bankAccount.deleteSuccess': 'Cuenta eliminada.',

  'nuevoVehiculo.noBankAccountAfterPublish.title': 'Vehículo publicado sin cuenta bancaria',
  'nuevoVehiculo.noBankAccountAfterPublish.description': 'Tu vehículo ya está publicado, pero no podrás recibir pagos hasta agregar una cuenta bancaria.',
  'nuevoVehiculo.noBankAccountAfterPublish.goHome': 'Ir al inicio',

  // --- Configuración ---
  'configuracion.title': 'Configuración',
  'configuracion.section.seguridad': 'Seguridad',
  'configuracion.section.preferencias': 'Preferencias',
  'configuracion.section.rentador': 'Rentador',
  'configuracion.section.cuenta': 'Cuenta',
  'configuracion.section.admin': 'Administración',
  'configuracion.admin.pricingZones': 'Mapa de precios dinámicos',
  'configuracion.verify.pending': 'Sin verificar',
  'configuracion.autoAccept.toast.on': 'Auto-aceptación activada',
  'configuracion.autoAccept.toast.off': 'Auto-aceptación desactivada',

  // --- Dashboard rentador ---
  'dashboard.title': 'Dashboard',
  'dashboard.greeting': 'Hola',
  'dashboard.ingresos': 'Ingresos del mes',
  'dashboard.reservasActivas': 'Reservas activas',
  'dashboard.vehiculosPublicados': 'Vehiculos publicados',
  'dashboard.calificacion': 'Calificacion promedio',
  'dashboard.ultimasReservas': 'Ultimas reservas',
  'dashboard.ocupacion': 'Ocupacion de flota',
  'dashboard.publicar': 'Publicar',
  'dashboard.tasaCancelacion': 'Tasa de cancelacion',
  'dashboard.activasHoy': 'Activas hoy',
  'dashboard.vehiculos': 'Vehiculos',
  'dashboard.ocupacionPorVehiculo': 'Ocupacion por vehiculo',
  'dashboard.masRentados': 'Mas rentados',
  'dashboard.bajaOcupacion': 'Baja ocupacion',
  'dashboard.bajaOcupacionHint': 'Pocos dias reservados para los proximos 30 dias. Proba promoverlo o ajustar su precio.',
  'dashboard.verRestantes': 'Ver los {n} restantes',
  'dashboard.verMenos': 'Ver menos',
  'dashboard.promover': 'Promover',
  'dashboard.ajustarPrecio': 'Ajustar precio',
  'dashboard.verDetalle': 'Ver detalle',
  'dashboard.periodo.week': 'Semana',
  'dashboard.periodo.month': 'Mes',
  'dashboard.periodo.quarter': 'Trimestre',
  'dashboard.periodo.custom': 'Personalizado',
  'dashboard.ingresos.week': 'Ingresos de la semana',
  'dashboard.ingresos.month': 'Ingresos del mes',
  'dashboard.ingresos.quarter': 'Ingresos del trimestre',
  'dashboard.ingresos.custom': 'Ingresos del periodo',
  'dashboard.custom.placeholder': 'Elegi el rango de fechas',
  'dashboard.custom.title': 'Periodo a analizar',
  'dashboard.reservasEnPeriodo': 'Reservas',
  'dashboard.sinVehiculos': 'Todavia no publicaste ningun vehiculo',
  'dashboard.sinVehiculosCta': 'Publica tu primer auto y empeza a generar ingresos.',
  'dashboard.sinReservas': 'Sin reservas en este periodo',
  'dashboard.detalle.title': 'Detalle del vehiculo',
  'dashboard.detalle.ingresos': 'Ingresos',
  'dashboard.detalle.ocupacion': 'Ocupacion',
  'dashboard.detalle.cancelaciones': 'Cancelaciones',
  'dashboard.detalle.reservas': 'Reservas en el periodo',
  'dashboard.detalle.calendario': 'Calendario de ocupacion',
  'dashboard.dias': 'dias',
  'dashboard.calendario.completo': 'Dia completo',
  'dashboard.calendario.parcial': 'Parcial',
  'dashboard.calendario.libre': 'Libre',
  'dashboard.calendario.libreTodoDia': 'Libre todo el dia',
  'dashboard.calendario.ocupadoTodoDia': 'Ocupado todo el dia',
  'dashboard.calendario.libreLabel': 'Horarios libres',
  'dashboard.error': 'No pudimos cargar las metricas. Intenta de nuevo.',

  // --- Mis vehiculos ---
  'misVehiculos.title': 'Mis vehiculos',
  'misVehiculos.add': 'Publicar vehiculo',
  'misVehiculos.empty': 'No publicaste vehiculos todavia',
  'misVehiculos.emptyAction': 'Publicar mi primer vehiculo',
  'misVehiculos.active': 'Activo',
  'misVehiculos.inactive': 'Inactivo',
  'misVehiculos.draft': 'Borrador',
  'editVehiculo.title': 'Editar vehiculo',
  'editVehiculo.subtitle': 'Edita fotos, caracteristicas y datos de tu vehiculo.',
  'editVehiculo.notFound': 'No encontramos ese vehiculo',
  'editVehiculo.backToList': 'Volver a mis vehiculos',
  'editVehiculo.noLocation': 'Sin ubicacion cargada',
  'editVehiculo.currentPrice': 'Precio actual',
  'editVehiculo.localDraft': 'Borrador local',
  'editVehiculo.formTitle': 'Detalles editables',
  'editVehiculo.formDescription': 'Completá los datos que querés actualizar de tu vehículo.',
  'editVehiculo.characteristicsDescription': 'Seleccioná las características que querés destacar en tu publicación.',
  'editVehiculo.photosTitle': 'Fotos del vehículo',
  'editVehiculo.photosDescription': 'Podés borrar fotos existentes y sumar nuevas antes de guardar.',
  'editVehiculo.addPhotos': 'Agregar fotos',
  'editVehiculo.noPhotos': 'Todavia no hay fotos cargadas',
  'editVehiculo.photoStored': 'Guardada',
  'editVehiculo.photoNew': 'Nueva',
  'editVehiculo.photoLimit': 'Podés subir hasta 10 fotos',
  'editVehiculo.photoMinimum': 'El vehículo debe tener al menos 3 fotos para poder guardar',
  'editVehiculo.photoMinimumHint': 'Se requieren al menos 3 fotos para guardar',
  'editVehiculo.removePhoto': 'Borrar foto',
  'editVehiculo.uploadingPhotos': 'Subiendo fotos nuevas...',
  'editVehiculo.saving': 'Guardando cambios...',
  'editVehiculo.deleteVehicle': 'Eliminar vehiculo',
  'editVehiculo.deleteConfirm': 'Vas a eliminar este vehiculo y todas sus fotos. Esta accion no se puede deshacer.',
  'editVehiculo.saveSuccess': 'Cambios publicados',
  'editVehiculo.saveError': 'No pudimos guardar los cambios',
  'editVehiculo.deleteSuccess': 'Vehiculo eliminado',
  'editVehiculo.section.details.title': 'Detalles del vehículo',
  'editVehiculo.section.details.summary.empty': 'Sin completar',
  'editVehiculo.section.price.title': 'Precio por día',
  'editVehiculo.section.price.description': 'Lo ven los conductores al buscar.',
  'editVehiculo.section.availability.title': 'Disponibilidad',
  'editVehiculo.section.availability.description': 'Desde cuándo se puede reservar, si está publicado y qué descuentos aplica.',
  'editVehiculo.section.availability.enabledHint': 'Visible en la búsqueda',
  'editVehiculo.section.availability.disabledHint': 'Oculto de la búsqueda',

  'editVehiculo.section.discounts.title': 'Descuentos por alquiler prolongado',
  'editVehiculo.section.discounts.description': 'Ofrecé descuentos automáticos para incentivar reservas más largas.',
  'editVehiculo.section.discounts.explanation': 'Los descuentos se aplican automáticamente cuando una reserva supera la cantidad mínima de días configurada.',

  'editVehiculo.section.discounts.howItWorks': '¿Cómo funcionan?',
  'editVehiculo.section.discounts.bestDiscountOnly': 'Se aplica únicamente el mejor descuento alcanzado.',
  'editVehiculo.section.discounts.mustIncrease': 'Los descuentos deben aumentar con la duración.',
  'editVehiculo.section.discounts.maxDiscountLimit': 'El descuento máximo permitido es 50%.',
  'editVehiculo.section.discounts.maxDiscountAvailable': 'Hasta {discount}% de descuento disponible',

  'editVehiculo.section.discounts.summary.none': 'Sin descuentos configurados',
  'editVehiculo.section.discounts.summary.maxDiscount': 'Hasta {discount}% de descuento',

  'editVehiculo.section.location.title': 'Ubicación',
  'editVehiculo.section.location.summary.empty': 'Sin ubicación cargada',
  'editVehiculo.section.features.title': 'Características',
  'editVehiculo.section.features.summary.empty': 'Sin características destacadas',
  'editVehiculo.section.rules.title': 'Reglas de reserva',
  'editVehiculo.section.rules.summary.shared': 'Set compartido: {name}',
  'editVehiculo.section.rules.summary.private': 'Reglas particulares activas',
  'editVehiculo.section.rules.summary.none': 'Sin reglas asignadas',
  'editVehiculo.section.description.title': 'Descripción',
  'editVehiculo.section.description.description': 'Contale al conductor por qué tu auto vale la pena.',
  'editVehiculo.section.description.summary.empty': 'Sin descripción',
  'editVehiculo.section.photos.summary': '{count} fotos cargadas',
  'editVehiculo.section.entrega.title': 'Entrega y retiro a domicilio',
  'editVehiculo.section.entrega.description': 'Ofrecé llevar y buscar el auto al domicilio del conductor.',
  'editVehiculo.section.entrega.summary.disabled': 'Sin servicio a domicilio',
  'editVehiculo.section.entrega.summary.deliveryOnly': 'Entrega: {fee}',
  'editVehiculo.section.entrega.summary.returnOnly': 'Retiro: {fee}',
  'editVehiculo.section.entrega.summary.both': 'Entrega: {deliveryFee} · Retiro: {returnFee}',
  'editVehiculo.section.entrega.summary.free': 'gratis',
  'editVehiculo.entrega.delivery.toggle': 'Entrega a domicilio',
  'editVehiculo.entrega.delivery.fee': 'Cargo por entrega (ARS)',
  'editVehiculo.entrega.delivery.feePlaceholder': '0 = gratis',
  'editVehiculo.entrega.return.toggle': 'Retiro a domicilio',
  'editVehiculo.entrega.return.fee': 'Cargo por retiro (ARS)',
  'editVehiculo.entrega.return.feePlaceholder': '0 = gratis',
  'editVehiculo.danger.title': 'Acciones peligrosas',
  'editVehiculo.field.brand': 'Marca',
  'editVehiculo.field.model': 'Modelo',
  'editVehiculo.field.year': 'Ano',
  'editVehiculo.field.plate': 'Patente',
  'editVehiculo.field.basePrice': 'Precio por dia',
  'editVehiculo.field.color': 'Color',
  'editVehiculo.field.mileage': 'Kilometraje',
  'editVehiculo.field.availableFrom': 'Disponible desde',
  'editVehiculo.field.enabled': 'Estado',
  'editVehiculo.field.isAccessible': 'Accesible',
  'editVehiculo.field.city': 'Ciudad',
  'editVehiculo.field.province': 'Provincia',
  'editVehiculo.field.description': 'Descripcion',
  'editVehiculo.saveHint': 'Guarda para publicar los cambios en el vehiculo.',
  'vehiculo.autoAccept.label': 'Auto-aceptar reservas',
  'vehiculo.autoAccept.descripcion': 'Decidí cómo se manejan las solicitudes para este vehículo.',
  'vehiculo.autoAccept.opcion.heredar': 'Heredar de mi configuración',
  'vehiculo.autoAccept.opcion.si': 'Auto-aceptar siempre',
  'vehiculo.autoAccept.opcion.no': 'Aprobar manualmente siempre',
  'vehiculo.discountTiers.title': 'Descuentos por duración',
  'vehiculo.discountTiers.description': 'Agregá tramos de alquiler para aplicar descuentos automáticos por cantidad de días.',
  'vehiculo.discountTiers.empty': 'Todavía no hay descuentos cargados.',
  'vehiculo.discountTiers.add': 'Agregar tramo',
  'vehiculo.discountTiers.tierLabel': 'Tramo {index}',
  'vehiculo.discountTiers.minimumDays': 'Desde cuántos días',
  'vehiculo.discountTiers.discountPercentage': 'Descuento (%)',
  'vehiculo.discountTiers.afterPrevious': 'Debe ser mayor que {days} días.',
  'vehiculo.discountTiers.validHint': 'Los tramos se ordenan automáticamente al guardar.',
  'vehiculo.discountTiers.invalidHint': 'Revisá que cada tramo tenga días y descuento válidos.',
  'vehiculo.discountTiers.summary': '{count} tramos cargados',
  'vehiculo.maxDiscount': 'Hasta {discount} de descuento',
  'nuevoVehiculo.title': 'Publicar vehiculo',
  'nuevoVehiculo.step.datos': 'Datos',
  'nuevoVehiculo.step.fotos': 'Fotos',
  'nuevoVehiculo.step.disponibilidad': 'Disponibilidad',
  'nuevoVehiculo.step.tarifas': 'Tarifas y descuentos',
  'nuevoVehiculo.step.publicar': 'Publicar',
  'nuevoVehiculo.publishing.title': 'Publicando vehículo',
  'nuevoVehiculo.publishing.description': 'Estamos subiendo las fotos y creando tu publicación. No cierres esta pantalla.',
  'nuevoVehiculo.toast.published': 'Vehículo publicado',

  // --- Favoritos ---
  'favoritos.loginHint': 'Guarda tus autos favoritos para reservar con tu cuenta',
  'favoritos.title': 'Mis favoritos',
  'favoritos.empty': 'Todavia no guardaste favoritos',
  'favoritos.emptyHint': 'Toca el corazon en cualquier vehiculo para guardarlo aca',
  'favoritos.emptyAction': 'Explorar vehiculos',
  'favoritos.count': 'guardados',
  'favoritos.countOne': 'guardado',
  'favoritos.add': 'Agregar a favoritos',
  'favoritos.remove': 'Quitar de favoritos',
  'favoritos.toast.added': 'Agregado a favoritos',
  'favoritos.toast.removed': 'Eliminado de favoritos',
  'favoritos.toast.undo': 'Deshacer',
  'favoritos.toast.error': 'No pudimos actualizar tus favoritos',

  // --- Soporte / FAQ ---
  'soporte.title': 'Soporte',
  'faq.title': 'Preguntas frecuentes',
  'faq.search.placeholder': 'Buscar preguntas...',
  'faq.search.noResults': 'No encontramos preguntas que coincidan con tu busqueda.',
  'faq.search.clearButton': 'Limpiar busqueda',
  'faq.categories.all': 'Todas',
  'faq.categories.reservas': 'Reservas',
  'faq.categories.pagos': 'Pagos y tarifas',
  'faq.categories.vehiculos': 'Vehiculos',
  'faq.categories.cuenta': 'Cuenta y verificacion',
  'faq.categories.disputas': 'Disputas y soporte',
  'soporte.contact.title': 'Necesitas mas ayuda?',
  'soporte.contact.chat': 'Chat en la app',
  'soporte.contact.whatsapp': 'WhatsApp',
  'soporte.contact.email': 'Email',
  'soporte.contact.hours': 'Lun-Vie 9:00 a 20:00 hs',

  // --- Errores API (por codigo) ---
  'error.unauthorized': 'Sesion expirada. Por favor ingresa nuevamente.',
  'error.forbidden': 'No tenes permiso para realizar esta accion.',
  'error.notFound': 'No encontramos lo que buscas.',
  'error.conflict': 'Ese periodo ya esta reservado.',
  'error.validation': 'Revisa los campos del formulario.',
  'error.emailUnverified': 'Verifica tu correo electronico antes de continuar.',
  'error.default': 'Algo salio mal. Intenta de nuevo.',
  'error.IDENTITY_VERIFICATION_REQUIRED': 'Necesitas verificar tu identidad antes de continuar.',
  'error.DRIVER_LICENSE_VERIFICATION_REQUIRED': 'Necesitas verificar tu licencia de conducir antes de continuar.',
  'error.ADMIN_FORBIDDEN': 'No tenés permisos de administrador.',
  'error.USER_HAS_VEHICLES': 'No podes eliminar tu cuenta mientras tengas vehiculos publicados.',
  'error.USER_HAS_ACTIVE_RESERVATIONS': 'No podes eliminar tu cuenta mientras tengas reservas activas.',
  'error.BANK_ACCOUNT_REQUIRED': 'Necesitas vincular una cuenta bancaria para recibir pagos.',
  'error.INSUFFICIENT_BALANCE': 'Saldo insuficiente para realizar esta operacion.',
  'error.INVALID_ENTITY_DATA': 'Algunos datos ingresados no son validos. Revisalos e intenta de nuevo.',
  'error.UPLOAD_FILE_TOO_LARGE': 'El archivo es demasiado grande. Elegi uno de menor tamano.',
  'error.UPLOAD_INVALID_TYPE': 'El tipo de archivo no es valido. Usa imagenes JPG, PNG o HEIC.',
  'error.RESERVATION_ALREADY_CANCELLED': 'Esta reserva ya fue cancelada.',
  'error.RESERVATION_TRANSFER_EXPIRED': 'Se vencio el tiempo para realizar la transferencia.',
  'error.VOUCHER_NOT_FOUND': 'No encontramos un voucher valido para esta reserva.',
  'error.VOUCHER_RESERVATION_CANCELLED': 'La reserva asociada a este voucher fue cancelada.',
  'error.RESERVATION_INVALID_QR_TOKEN': 'El codigo QR no es valido. Verifica con el conductor o rentador.',
  'error.RESERVATION_DEPOSIT_NOT_AVAILABLE': 'El deposito no esta disponible para esta reserva.',
  'error.RESERVATION_BALANCE_NOT_DUE': 'El saldo de esta reserva no esta pendiente de pago.',
  'error.RESERVATION_BALANCE_OVERDUE': 'El saldo de esta reserva esta vencido.',
  'error.DEPOSIT_PERCENTAGE_OUT_OF_RANGE': 'El porcentaje de deposito debe estar entre 10% y 50%.',
  'error.RULESET_NOT_FOUND_FOR_OWNER': 'No encontramos este set de reglas para tu cuenta.',
  'error.RULESET_PRIVATE_CANNOT_BE_SHARED': 'Este set de reglas es privado y no puede compartirse.',
  'error.RULESET_VEHICLE_ID_IMMUTABLE': 'No se puede cambiar el vehiculo asociado a este set de reglas.',
  'error.VEHICLE_ALREADY_HAS_PRIVATE_RULESET': 'Este vehiculo ya tiene un set de reglas particular.',
  'error.VEHICLE_ALREADY_PROMOTED': 'Este vehiculo ya esta promocionado.',
  'error.INVALID_MAP_BOUNDS': 'Los limites del mapa no son validos.',
  'error.VEHICLE_LOCATION_REQUIRED': 'El vehiculo necesita una ubicacion para ser publicado.',
  'error.CHAT_NOT_ALLOWED': 'El chat esta disponible solo para reservas confirmadas o en curso.',
  'error.TICKET_NOT_FOUND': 'No encontramos el ticket de soporte.',
  'error.TICKET_ALREADY_EXISTS': 'Ya existe un ticket abierto para esta reserva.',
  'error.TICKET_RESERVATION_INVALID_STATUS': 'No se puede abrir un ticket para una reserva en este estado.',
  'error.INVALID_WITHDRAW_AMOUNT': 'El monto de retiro no es valido.',
  'error.INTERNAL_ERROR': 'No pudimos procesar tu solicitud. Intenta nuevamente en unos minutos.',
  'error.FAVORITE_ALREADY_EXISTS': 'Este vehiculo ya esta en tus favoritos.',
  'error.FAVORITE_NOT_FOUND': 'Este vehiculo no esta en tus favoritos.',
  'error.UNSUPPORTED_PAYMENT_METHOD': 'El metodo de pago seleccionado no esta soportado.',
  'error.PAYMENT_METHOD_NOT_FOUND': 'No encontramos ese metodo de pago.',
  'error.BULK_PRICE_VEHICLE_NOT_OWNED': 'Uno de los vehiculos seleccionados ya no te pertenece.',
  'error.BULK_PRICE_RESULT_INVALID': 'El ajuste daria un precio invalido en algun vehiculo.',
  'error.RESERVATION_CANCEL_EXTENSION_NOT_ALLOWED': 'No se puede cancelar una extension en este estado.',
  'error.RESERVATION_EXTENSION_ALREADY_PENDING': 'Ya hay una extension pendiente para esta reserva.',
  'error.RESERVATION_EXTENSION_NOT_PENDING': 'No hay una extension pendiente para cancelar.',

  // --- Reglas de Reserva ---
  'reservationRules.title': 'Reglas de reserva',
  'reservationRules.description': 'Crea y gestiona sets de reglas reutilizables para tus vehículos',
  'reservationRules.create': 'Crear set',
  'reservationRules.createNew': 'Nuevo set de reglas',
  'reservationRules.edit': 'Editar set',
  'reservationRules.editTitle': 'Editar set de reglas',
  'reservationRules.editDescription': 'Actualiza los términos. Los cambios aplican a reservas futuras.',
  'reservationRules.name': 'Nombre del set',
  'reservationRules.namePlaceholder': 'Ej: Flexible sin depósito',
  'reservationRules.createDescription': 'Descripción (opcional)',
  'reservationRules.descriptionPlaceholder': 'Detalles adicionales para recordar qué aplica este set',
  'reservationRules.cancellationPolicy': 'Política de cancelación',
  'reservationRules.cancellation.flexible': 'Flexible',
  'reservationRules.cancellation.flexibleDesc': 'Reembolso total hasta 24 horas antes del alquiler',
  'reservationRules.cancellation.moderate': 'Moderada',
  'reservationRules.cancellation.moderateDesc': '50% de reembolso hasta 48 horas antes',
  'reservationRules.cancellation.strict': 'Estricta',
  'reservationRules.cancellation.strictDesc': 'Sin posibilidad de reembolso',
  'reservationRules.deposit': 'Seña',
  'reservationRules.deposit.label': 'Seña',
  'reservationRules.deposit.enable': 'Requerir seña',
  'reservationRules.deposit.sliderHint': 'Porcentaje entre 10% y 50%',
  'reservationRules.deposit.none': 'Sin seña',
  'reservationRules.deposit.formatted': '{percentage}%',
  'reservationRules.scope.title': 'Cómo querés guardar este set',
  'reservationRules.scope.sharedTitle': 'Como set reutilizable',
  'reservationRules.scope.sharedDescription':
    'Va a tu lista de sets y podés aplicarlo a otros vehículos.',
  'reservationRules.scope.privateTitle': 'Solo para este vehículo',
  'reservationRules.scope.privateDescription':
    'Sólo aplica a este vehículo y no se mezcla con tus sets reutilizables.',
  'reservationRules.scope.rememberChoice': 'No volver a preguntar',
  'reservationRules.scope.privateNote': 'Reglas particulares de este vehículo',
  'reservationRules.selector.createNew': '+ Crear nuevo set',
  'reservationRules.selector.none': 'Sin set compartido asignado',
  'reservationRules.selector.sharedLabel': 'Set compartido (opcional)',
  'reservationRules.maxKilometrage': 'Kilometraje máximo',
  'reservationRules.kilometrage.unlimited': 'Sin límite de km',
  'reservationRules.kilometrage.limited': 'Limitado a',
  'reservationRules.rentalTime': 'Restricciones de tiempo',
  'reservationRules.rentalTime.noConstraints': 'Sin restricciones',
  'reservationRules.rentalTime.minDays': 'Mínimo de días',
  'reservationRules.rentalTime.maxDays': 'Máximo de días',
  'reservationRules.minDays': 'Días mínimos (opcional)',
  'reservationRules.maxDays': 'Días máximos (opcional)',
  'reservationRules.save': 'Guardar cambios',
  'reservationRules.delete': 'Eliminar set',
  'reservationRules.confirmDelete': '¿Estás seguro? Este set de reglas se eliminará, pero los vehículos asignados no se verán afectados.',
  'reservationRules.created': 'Set de reglas creado',
  'reservationRules.updated': 'Set de reglas actualizado',
  'reservationRules.deleted': 'Set de reglas eliminado',
  'reservationRules.assigned': 'Reglas asignadas al vehículo',
  'reservationRules.empty': 'No creaste sets de reglas todavía',
  'reservationRules.createFirst': 'Crear primer set',
  'reservationRules.assignedTo': 'Asignado a vehículo(s)',
  'reservationRules.notAssigned': 'No asignado a ningún vehículo',
  'reservationRules.vehicle': 'vehículo',
  'reservationRules.vehicles': 'vehículos',
  'reservationRules.nameRequired': 'El nombre del set es requerido',

  // --- Chat interno ---
  'chat.title': 'Chat',
  'chat.empty': 'No hay mensajes todavía. ¡Mandá el primero!',
  'chat.input.placeholder': 'Escribí un mensaje...',
  'chat.input.send': 'Enviar',
  'chat.error.load': 'No pudimos cargar los mensajes. Intentá de nuevo.',
  'chat.error.send': 'No se pudo enviar el mensaje. Intentá de nuevo.',
  'chat.error.notAllowed': 'El chat está disponible solo para reservas confirmadas o en curso.',
  'chat.whatsapp.cta': 'Contactar a {name} por WhatsApp',
  'chat.whatsapp.ariaLabel': 'Abrir chat de WhatsApp con {name}',
  'reservas.detail.actions.chat': 'Abrir chat',

  // --- Documentación del vehículo ---
  'documentosVehiculo.title': 'Documentación del vehículo',
  'documentosVehiculo.status.none': 'Sin documentación',
  'documentosVehiculo.status.pending': 'Pendiente de aprobación',
  'documentosVehiculo.status.verified': 'Documentación verificada',
  'documentosVehiculo.status.rejected': 'Documentación rechazada',
  'documentosVehiculo.form.title': 'Subí la documentación del vehículo',
  'documentosVehiculo.form.copy': 'Necesitamos estos documentos para verificar el vehículo y habilitarlo en la plataforma.',
  'documentosVehiculo.form.titleDoc': 'Título del vehículo',
  'documentosVehiculo.form.greenCard': 'Cédula verde',
  'documentosVehiculo.form.titleHelper': 'Foto del título de propiedad, ambas caras.',
  'documentosVehiculo.form.greenCardHelper': 'Foto de la cédula verde, ambas caras.',
  'documentosVehiculo.form.submit': 'Enviar documentación',
  'documentosVehiculo.form.submitting': 'Enviando...',
  'documentosVehiculo.form.exit': 'Salir',
  'documentosVehiculo.form.acceptedCopy': 'Al enviar aceptas que revisemos los documentos para validar el vehículo.',
  'documentosVehiculo.pending.title': 'Documentación en revisión',
  'documentosVehiculo.pending.copy': 'La documentación está siendo procesada. No hace falta que esperes. El estado se actualizará automáticamente.',
  'documentosVehiculo.verified.copy': 'La documentación del vehículo fue verificada correctamente. Ya puede alquilarse en la plataforma.',
  'documentosVehiculo.verified.cta': 'Volver a mis vehículos',
  'documentosVehiculo.rejected.copy': 'Hubo un problema con la documentación. Revisá el motivo y volvé a enviar.',
  'documentosVehiculo.rejected.reasonTitle': 'Motivo del rechazo',
  'documentosVehiculo.toast.submitted': 'Documentación enviada',
  'documentosVehiculo.toast.success': 'Documentación verificada',
  'documentosVehiculo.guard.title': 'Falta la documentación del vehículo',
  'documentosVehiculo.guard.copy': 'Para publicar un vehículo necesitás subir la documentación primero.',
  'documentosVehiculo.guard.cta': 'Ir a subir documentación',
  'documentosVehiculo.section.summary.verified': 'Verificada',
  'documentosVehiculo.section.summary.pending': 'Pendiente de aprobación',
  'documentosVehiculo.section.summary.rejected': 'Rechazada - Revisar',
  'documentosVehiculo.section.summary.none': 'Pendiente de carga',

  // --- Promocionar ---
  'promocionar.title': 'Promocionar vehículo',
  'promocionar.description': 'Dale más visibilidad a tu vehículo para aparecer primero en las búsquedas de los conductores.',
  'promocionar.duration': 'Duración de la promoción',
  'promocionar.total': 'Costo total',
  'promocionar.promote': 'Promocionar por {amount}',
  'promocionar.promoting': 'Promocionando...',
  'promocionar.success': 'Vehículo promocionado con éxito',
  'promocionar.active': 'Promocionado',
  'promocionar.expiresAt': 'Vence el {date}',
  'promocionar.noPromotion': 'Sin promoción activa',
  'promocionar.durationDays': '{days} días',
  'promocionar.error': 'No pudimos promocionar el vehículo. Intentá de nuevo.',
  'promocionar.loading': 'Cargando planes...',
  'promocionar.paymentMethod': 'Medio de pago',
  'promocionar.paymentMethod.credit_card': 'Tarjeta de crédito',
  'promocionar.paymentMethod.debit_card': 'Tarjeta de débito',
  'promocionar.paymentMethod.bank_transfer': 'Transferencia bancaria',
  'promocionar.paymentMethod.digital_wallet': 'Billetera virtual',
  'promocionar.paymentMethod.walletProvider': 'Seleccioná tu billetera',
  'promocionar.paymentMethod.mercadopago': 'Mercado Pago',
  'promocionar.paymentMethod.uala': 'Ualá',
  'promocionar.transfer.pending': 'Esperando confirmación de la transferencia...',
  'promocionar.transfer.alias': 'Alias: {alias}',
  'promocionar.transfer.code': 'CBU/CVU: {code}',
  'promocionar.transfer.expiresAt': 'Vence en aproximadamente 2 horas',
  'promocionar.paidAt': 'Pagado el {date}',
  'promocionar.transactionId': 'Transacción: {id}',

  // --- Ajuste masivo de precios ---
  'bulkPrice.enterSelectionMode': 'Editar precios',
  'bulkPrice.exitSelectionMode': 'Cancelar',
  'bulkPrice.selectedCount': '{count} de {total} seleccionados',
  'bulkPrice.selectAll': 'Seleccionar todos',
  'bulkPrice.deselectAll': 'Deseleccionar todos',
  'bulkPrice.continue': 'Continuar',
  'bulkPrice.dialogTitle': 'Ajustar precio en lote',
  'bulkPrice.operationLegend': 'Tipo de operación',
  'bulkPrice.operationSet': 'Establecer precio fijo',
  'bulkPrice.operationPercentage': 'Ajustar por porcentaje',
  'bulkPrice.inputAmountLabel': 'Nuevo precio (pesos)',
  'bulkPrice.inputPercentageLabel': 'Porcentaje (ej: 15 para +15%, -10 para -10%)',
  'bulkPrice.previewTitle': 'Vista previa del ajuste',
  'bulkPrice.activeReservationsWarning': '{count} reservas activas mantendrán el precio anterior',
  'bulkPrice.checkingReservations': 'Verificando reservas activas…',
  'bulkPrice.invalidPriceRow': 'Precio inválido',
  'bulkPrice.confirm': 'Confirmar {count} cambios',
  'bulkPrice.viewPreview': 'Ver vista previa',
  'bulkPrice.successToast': '{count} vehículos actualizados',
  'bulkPrice.errors.notOwned': 'Uno de los vehículos seleccionados ya no te pertenece.',
  'bulkPrice.errors.resultInvalid': 'El ajuste daría precio cero o negativo en algún vehículo.',

  // --- Generales ---
  'general.loading': 'Cargando...',
  'general.retry': 'Reintentar',
  'general.cancel': 'Cancelar',
  'general.confirm': 'Confirmar',
  'general.yes': 'Sí',
  'general.no': 'No',
  'general.save': 'Guardar',
  'general.saving': 'Guardando...',
  'general.edit': 'Editar',
  'general.delete': 'Eliminar',
  'general.back': 'Volver',
  'general.next': 'Siguiente',
  'general.previous': 'Anterior',
  'general.close': 'Cerrar',
  'general.seeAll': 'Ver todo',
  'general.search': 'Buscar',

  // --- Reservar (flujo de reserva) ---
  'reservar.title': 'Reservar vehiculo',
  'reservar.step.fechas': 'Fechas y horarios',
  'reservar.step.contrato': 'Contrato',
  'reservar.step.pago': 'Pago',
  'reservar.pickupAt': 'Retiro',
  'reservar.returnAt': 'Devolucion',
  'reservar.invalidRange': 'La fecha de devolucion debe ser posterior a la de retiro',
  'reservar.rules.title': 'Reglas de reserva',
  'reservar.rules.deposit': 'Depósito',
  'reservar.rules.maxKilometrage': 'Kilometraje máximo',
  'reservar.rules.minDuration': 'Duración mínima',
  'reservar.rules.maxDuration': 'Duración máxima',
  'reservar.rules.cancellationPolicy': 'Política de cancelación',
  'reservar.rules.noMinimum': 'Sin mínimo',
  'reservar.rules.noMaximum': 'Sin máximo',
  'reservar.rules.collapse': 'Marcar como leído y minimizar',
  'reservar.rules.markAsRead': 'Marcar como leídas',
  'reservar.rules.read': 'Reglas leídas',
  'reservar.rules.expand': 'Expandir reglas',
  'reservar.durationError.min': 'La reserva debe durar al menos {days}.',
  'reservar.durationError.max': 'La reserva no puede durar más de {days}.',
  'reservar.summary': 'Resumen',
  'reservar.total': 'Total estimado',
  'reservar.breakdown.subtitle': 'Desglose del precio',
  'reservar.breakdown.subtotal': 'Subtotal',
  'reservar.breakdown.discount': 'Descuento',
  'reservar.breakdown.savings': 'Ahorro',
  'reservar.breakdown.appliedTier': 'Aplica desde {days} días (-{percentage}%)',
  'reservar.breakdown.levelDiscount': 'Desc. por nivel ({pct}%)',
  'reservar.contract.title': 'Contrato digital',
  'reservar.contract.body':
    'Al continuar declaras que leiste y aceptas las condiciones del alquiler (estado del vehiculo, deposito, devolucion en tiempo y forma, etc.).',
  'reservar.contract.accept': 'Acepto los terminos del contrato digital',
  'reservar.paymentMethod': 'Medio de pago',
  'reservar.paymentMethod.credit_card': 'Tarjeta de credito',
  'reservar.paymentMethod.debit_card': 'Tarjeta de debito',
  'reservar.paymentMethod.bank_transfer': 'Transferencia bancaria',
  'reservar.paymentMethod.digital_wallet': 'Billetera virtual',
  'reservar.continueToContract': 'Continuar',
  'reservar.continueToPayment': 'Continuar al pago',
  'reservar.payAndConfirm': 'Pagar y confirmar',
  'reservar.holdNotice': 'Te reservamos el vehiculo durante 10 minutos para que completes el pago.',
  'reservar.busyTitle': 'Fechas ya ocupadas para este vehiculo:',
  'reservar.overlapsBusy': 'El rango seleccionado se superpone con una reserva existente.',
  'reservar.holdExpiresIn': 'Tiempo restante:',
  'reservar.success': 'Reserva confirmada',
  'reservar.reReservar.boton': 'Re-reservar',
  'reservar.reReservar.noDisponible': 'Este vehículo ya no está disponible. Te mostramos opciones similares.',
  'reservar.errors.RESERVATION_VEHICLE_NOT_AVAILABLE': 'Este vehiculo ya no esta disponible en esas fechas.',
  'reservar.errors.RESERVATION_HOLD_EXPIRED': 'Se vencio el tiempo para pagar. Tu reserva fue cancelada.',
  'reservar.errors.RESERVATION_CONTRACT_NOT_ACCEPTED': 'Tenes que aceptar el contrato para continuar.',
  'reservar.errors.RESERVATION_INVALID_TRANSITION': 'La reserva ya no admite esta accion.',
  'reservar.errors.RESERVATION_NOT_FOUND': 'No encontramos la reserva.',
  'reservar.errors.RESERVATION_FORBIDDEN': 'No tenes permiso sobre esta reserva.',
  'reservar.errors.RESERVATION_OWNER_CANNOT_RESERVE': 'No podes reservar tu propio vehiculo.',
  'reservar.errors.generic': 'No pudimos completar la reserva. Intentalo nuevamente.',
  'reservar.errors.VEHICLE_HOME_DELIVERY_NOT_ENABLED': 'Este vehículo no tiene entrega a domicilio habilitada.',
  'reservar.errors.VEHICLE_HOME_RETURN_NOT_ENABLED': 'Este vehículo no tiene retiro a domicilio habilitado.',
  'reservar.errors.HOME_DELIVERY_ADDRESS_REQUIRED': 'Ingresá la dirección de entrega.',
  'reservar.errors.HOME_RETURN_ADDRESS_REQUIRED': 'Ingresá la dirección de retiro.',
  'reservar.quote.validFor.label': 'Tiempo para completar tu reserva',
  'reservar.quote.refreshing': 'Actualizando precio…',
  'reservar.quote.priceChanged': 'Precio actualizado: {old} → {new}',
  'reservar.quote.expired.title': 'Tiempo finalizado',
  'reservar.quote.expired.message': 'Estamos recalculando el valor de tu reserva…',
  'reservar.errors.PRICE_QUOTE_NOT_FOUND': 'No encontramos la cotización. Volvé a cotizar.',
  'reservar.errors.PRICE_QUOTE_EXPIRED': 'La cotización venció. Volvé a cotizar para confirmar el precio.',
  'reservar.errors.PRICE_QUOTE_VEHICLE_MISMATCH': 'La cotización no corresponde a este vehículo.',
  'reservar.errors.PRICE_QUOTE_CONDUCTOR_MISMATCH': 'Esta cotización ya está asociada a otro conductor. Volvé a cotizar.',
  'reservar.address.original': 'Dirección original',
  'reservar.address.custom': 'Personalizada',
  'reservar.homeDelivery.sectionTitle': 'Entrega del vehículo',
  'reservar.homeDelivery.toggle': 'Entrega a domicilio',
  'reservar.homeDelivery.fee': 'Cargo: {fee}',
  'reservar.homeDelivery.freeLabel': 'Gratis',
  'reservar.homeDelivery.addressLabel': 'Dirección de entrega',
  'reservar.homeDelivery.addressPlaceholder': 'Ej: Av. Corrientes 1234, CABA',
  'reservar.homeReturn.sectionTitle': 'Devolución del vehículo',
  'reservar.homeReturn.toggle': 'Retiro a domicilio',
  'reservar.homeReturn.fee': 'Cargo: {fee}',
  'reservar.homeReturn.addressLabel': 'Dirección de retiro',
  'reservar.homeReturn.addressPlaceholder': 'Ej: Av. Corrientes 1234, CABA',
  'reservar.homeReturn.sameAsDelivery': 'Misma que la de entrega',
  'reservar.homeReturn.changeDisabled': 'El rentador no habilitó el cambio de dirección de devolución',
  'reservar.homeDelivery.vehicleAddress': 'Dirección del vehículo: {address}',
  'reservar.breakdown.baseRent': 'Alquiler base',
  'reservar.breakdown.homeDeliveryFee': 'Cargo entrega a domicilio',
  'reservar.breakdown.homeReturnFee': 'Cargo retiro a domicilio',

  // --- Medios de Pago ---
  'paymentMethods.title': 'Medios de pago',
  'paymentMethods.add': 'Agregar medio de pago',
  'paymentMethods.empty': 'No tenés medios de pago guardados',
  'paymentMethods.emptyHint': 'Agregá una tarjeta o billetera virtual para pagar tus reservas.',
  'paymentMethods.card': 'Tarjeta',
  'paymentMethods.wallet': 'Billetera virtual',
  'paymentMethods.lastFour': 'Terminada en {lastFour}',
  'paymentMethods.edit': 'Editar',
  'paymentMethods.delete': 'Eliminar',
  'paymentMethods.save': 'Guardar',
  'paymentMethods.createSuccess': 'Medio de pago agregado',
  'paymentMethods.updateSuccess': 'Medio de pago actualizado',
  'paymentMethods.deleteSuccess': 'Medio de pago eliminado',
  'paymentMethods.deleteWarning': 'Estás a punto de eliminar tu último medio de pago. Quedarás sin medios de pago registrados.',
  'paymentMethods.deleteConfirm': 'Eliminar de todas formas',
  'paymentMethods.form.type': 'Tipo de pago',
  'paymentMethods.form.cardBrand': 'Marca (ej. VISA)',
  'paymentMethods.form.lastFour': 'Últimos 4 dígitos',
  'paymentMethods.form.expMonth': 'Mes (MM)',
  'paymentMethods.form.expYear': 'Año (AAAA)',
  'paymentMethods.form.cardholderName': 'Nombre del titular',
  'paymentMethods.form.walletProvider': 'Billetera',
  'paymentMethods.form.walletAlias': 'Alias / CVU',
  'paymentMethods.form.cancel': 'Cancelar',

  'reservas.detail.extend.cta': 'Extender alquiler',
  'reservas.detail.extend.modalTitle': 'Extender alquiler',
  'reservas.detail.extend.modifyTitle': 'Modificar extensión',
  'reservas.detail.extend.saveChanges': 'Guardar cambios',
  'reservas.detail.extend.cancelPedido.cta': 'Cancelar pedido pendiente',
  'reservas.detail.extend.cancelPedido.success': 'Pedido de extensión cancelado',
  'reservas.detail.extend.currentEndAt': 'Devolución actual',
  'reservas.detail.extend.newEndAtLabel': 'Nueva fecha de devolución',
  'reservas.detail.extend.hourLabel': 'Hora',
  'reservas.detail.extend.minuteLabel': 'Min',
  'reservas.detail.extend.totalPreview': 'Costo',
  'reservas.detail.extend.pendingApproval': 'Extensión pendiente de aprobación',
  'reservas.detail.extend.pendingPayment': 'Extensión pendiente de pago',
  'reservas.detail.extend.pendingUntil': 'hasta',
  'reservas.detail.extend.infoInmediato':
    'Se aplicará inmediatamente. Se cobrará al medio de pago usado en el alquiler original.',
  'reservas.detail.extend.infoSolicitud':
    'Esta extensión necesita aprobación del rentador. Vas a recibir una respuesta dentro de las próximas 24 horas.',
  'reservas.detail.extend.infoLoading':
    'Calculando las condiciones de la extensión...',
  'reservas.detail.extend.infoVehicleError':
    'No pudimos cargar las condiciones del vehículo. Si confirmás, la extensión se enviará como solicitud para que el rentador la apruebe.',
  'reservas.detail.extend.confirmInmediato': 'Extender y pagar',
  'reservas.detail.extend.confirmSolicitud': 'Enviar solicitud',
  'reservas.detail.extend.cancel': 'Cancelar',
  'reservas.detail.extend.submitting': 'Enviando...',
  'reservas.detail.extend.success.inmediato':
    'Extensión aplicada. Estamos procesando el pago.',
  'reservas.detail.extend.success.solicitud':
    'Solicitud enviada. El rentador tiene 24 horas para responder.',
  'reservas.detail.extend.error.invalidEndAt':
    'La nueva fecha debe ser posterior a la devolución actual.',
  'reservas.detail.extend.error.generic':
    'No pudimos procesar la extensión. Intentalo nuevamente.',
  'reservas.detail.extend.error.vehicleNotAvailable':
    'El vehículo no está disponible en esas fechas. Elegí una fecha anterior.',
  'reservas.detail.extend.availableUntil': 'Disponible hasta el',

  'reservas.approve.extensionTitle': 'Aprobar solicitud de extensión',
  'reservas.approve.extensionSubtitle':
    'El conductor pidió extender un alquiler en curso. Revisá las nuevas fechas antes de aprobar.',
  'reservas.approve.extensionBody':
    'El conductor recibirá 10 minutos para pagar la extensión. Las solicitudes solapadas se van a rechazar automáticamente.',

  'reservar.errors.RESERVATION_EXTENSION_NOT_IN_PROGRESS':
    'Solo podés extender un alquiler en curso.',
  'reservar.errors.RESERVATION_EXTENSION_INVALID_END_AT':
    'La nueva fecha no es válida — tiene que ser posterior a la devolución actual.',

  // --- Historial ---
  'historial.title': 'Historial',
  'historial.empty': 'No tenés reservas pasadas.',
  'historial.error': 'No pudimos cargar tu historial. Intenta de nuevo.',
  'historial.cargando': 'Cargando historial',
  'historial.filtros.estado': 'Estado',
  'historial.filtros.desde': 'Desde',
  'historial.filtros.hasta': 'Hasta',
  'historial.filtros.limpiar': 'Limpiar filtros',
  'historial.filtros.title': 'Elegí un rango',
  'historial.filtros.todos': 'Todos',
  'historial.resenas.title': 'Reseñas recibidas',
  'historial.resenas.empty': 'Todavía no tenés reseñas.',
  'historial.resenas.emptyHint': 'Cuando un conductor complete un alquiler y deje una opinión, la vas a ver acá.',
  'historial.resenas.targetType.vehicle': 'Sobre vehículo',
  'historial.resenas.targetType.rentador': 'Sobre ti',
  'historial.resenas.targetType.conductor': 'Sobre conductor',
  'historial.resenas.error': 'No pudimos cargar tus reseñas. Intenta de nuevo.',
  'historial.resenas.cargando': 'Cargando reseñas',
  'historial.resenas.rating': 'Calificación',

  // --- Reseñas en detalle de reserva ---
  'reservas.detail.review.title': 'Reseña del conductor',
  'reservas.detail.review.empty': 'Sin reseña',

  // --- Tickets (US-36) ---
  'tickets.reportar.title': 'Reportar problema',
  'tickets.reportar.descriptionLabel': 'Describí el problema',
  'tickets.reportar.descriptionPlaceholder': 'Contanos qué pasó con el vehículo...',
  'tickets.reportar.fotosLabel': 'Adjuntar fotos (opcional, máx 5)',
  'tickets.reportar.cta': 'Enviar reporte',
  'tickets.reportar.sending': 'Enviando...',
  'tickets.reportar.uploadError': 'No se pudieron subir las fotos. Intentá de nuevo.',
  'tickets.reportar.success': 'Reporte enviado. La contraparte fue notificada.',
  'tickets.reportar.errorDuplicate': 'Ya tenés un reporte abierto para esta reserva.',
  'tickets.misTickets.title': 'Mis reportes',
  'tickets.misTickets.empty': 'No tenés reportes registrados.',
  'tickets.misTickets.reservaPrefix': 'Reserva #',
  'tickets.status.open': 'Abierto',
  'tickets.status.under_review': 'En revisión',
  'tickets.status.resolved': 'Resuelto',
  'tickets.status.closed': 'Cerrado',
  'tickets.reserva.tuReporte': 'Tu reporte',
  'tickets.reserva.reporteContraparte': 'Reporte de la contraparte',
  'tickets.contra.title': 'Reportados contra mí',
  'tickets.contra.empty': 'No tenés reportes en contra.',
  'perfil.reportes.title': 'Mis incidentes',
  'perfil.reportes.row': 'Incidentes',
  'perfil.reportes.tabMios': 'Generados por mí',
  'perfil.reportes.tabContra': 'Contra mí',

  // --- Precios dinámicos (rentador) ---
  'rentador.vehiculo.dynamicPricing.label': 'Precios dinámicos',
  'rentador.vehiculo.dynamicPricing.tooltip':
    'Los precios se ajustan automáticamente según la demanda de tu zona y de tu vehículo, dentro de un rango de 70-200% del precio base.',
  'rentador.vehiculo.dynamicPricing.activo': 'Precios dinámicos activos',
  'rentador.vehiculo.dynamicPricing.estado.activo': 'Activo',
  'rentador.vehiculo.dynamicPricing.estado.inactivo': 'Inactivo',
  'rentador.vehiculo.dynamicPricing.activoHint':
    'El precio se ajusta automáticamente según la demanda de tu zona (70–200% del base).',
  'rentador.vehiculo.dynamicPricing.inactivoHint':
    'Se cobra siempre el precio base sin importar la demanda de la zona.',

  // --- Admin: portal centralizado ---
  'admin.portal.title': 'Panel de administrador',
  'admin.portal.back': 'Salir',
  'admin.nav.tickets': 'Tickets',
  'admin.nav.pricing': 'Precios',
  'admin.nav.perfil': 'Perfil',
  'admin.floatingBtn.label': 'Panel de administrador',
  'admin.perfil.role': 'Administrador',
  'admin.perfil.reputation.title': 'Reputación del servicio',
  'admin.perfil.reputation.subtitle': 'Promedio de calificaciones de usuarios en tickets resueltos.',
  'admin.perfil.reputation.outOf': '/ 5',
  'admin.perfil.session.title': 'Sesión activa',
  'admin.perfil.session.logout': 'Cerrar sesión',
  'admin.tickets.reservaAsociada': 'Reserva asociada',
  'admin.tickets.acciones': 'Acciones de admin',

  // --- Admin: timeline de reserva ---
  'admin.reserva.timeline.title': 'Historial de la reserva',
  'admin.reserva.timeline.created': 'Reserva creada',
  'admin.reserva.timeline.contractAccepted': 'Contrato aceptado',
  'admin.reserva.timeline.depositPaid': 'Seña pagada',
  'admin.reserva.timeline.paid': 'Pago confirmado — reserva activa',
  'admin.reserva.timeline.balancePaid': 'Saldo pagado — reserva activa',
  'admin.reserva.timeline.started': 'Retiro confirmado — alquiler en curso',
  'admin.reserva.timeline.completed': 'Devolución confirmada',
  'admin.reserva.timeline.cancelled': 'Reserva cancelada',
  'admin.reserva.timeline.expired': 'Hold de pago vencido',

  // --- Admin: acceso restringido ---
  'admin.gate.deniedTitle': 'Acceso restringido',
  'admin.gate.deniedMessage': 'Esta sección es solo para administradores de la plataforma.',
  'admin.gate.backHome': 'Volver al inicio',

  // --- Admin: mapa de precios dinámicos ---
  'admin.pricing.titulo': 'Mapa de precios dinámicos',
  'admin.pricing.subtitulo': 'Oferta, demanda y multiplier promedio por zona.',
  'admin.pricing.cargando': 'Cargando zonas...',
  'admin.pricing.errorCarga': 'No pudimos cargar las zonas. Intentá de nuevo.',
  'admin.pricing.sinZonas': 'Todavía no hay actividad para mostrar.',
  'admin.pricing.demanda': 'Demanda (últimos 7 días)',
  'admin.pricing.oferta': 'Oferta',
  'admin.pricing.multiplierPromedio': 'Multiplier promedio',
  'admin.pricing.sinDatos': 'Sin actividad en esta zona',
  'admin.pricing.drawer.title': 'Detalle de la zona',
  'admin.pricing.drawer.cell': 'Celda H3',
  'admin.pricing.drawer.ratio': 'Demanda / Oferta',
  'admin.pricing.drawer.topVehiculos': 'Vehículos de muestra',
  'admin.pricing.drawer.close': 'Cerrar',
  'admin.pricing.unit.vehicles': 'autos',
  'admin.pricing.unit.signals': 'señales',
  'admin.pricing.enZona': 'En esta zona',
  'admin.pricing.headline.highDemand': 'Demanda ×{ratio} la oferta',
  'admin.pricing.headline.lowDemand': 'Oferta sobra para la demanda actual',
  'admin.pricing.headline.demandNoSupply': 'Hay demanda pero no hay autos publicados',
  'admin.pricing.headline.supplyIdle': 'Hay autos pero sin demanda registrada',
  'admin.pricing.legend.title': 'Multiplier',
  'admin.pricing.legend.veryHigh': 'Demanda muy alta (≥1.5×)',
  'admin.pricing.legend.high': 'Demanda alta (≥1.3×)',
  'admin.pricing.legend.elevated': 'Demanda elevada (≥1.15×)',
  'admin.pricing.legend.normal': 'Tarifa base',

  // --- Tickets US-59: subject + soporte general ---
  'tickets.reportar.subjectLabel': 'Asunto',
  'tickets.reportar.subjectPlaceholder': 'Resumí el problema en una línea',
  'tickets.tipo.vehicle_issue': 'Problema con vehículo',
  'tickets.tipo.counterpart_report': 'Reporte de contraparte',
  'tickets.tipo.support_request': 'Consulta a soporte',
  'tickets.detail.title': 'Detalle del ticket',
  'tickets.detail.infoSection': 'Información',
  'tickets.detail.subject': 'Asunto',
  'tickets.detail.description': 'Descripción',
  'tickets.detail.reserva': 'Reserva',
  'tickets.detail.noReserva': 'Sin reserva asociada',
  'tickets.detail.rating.title': 'Calificá la atención',
  'tickets.detail.rating.sent': 'Gracias por tu calificación',
  'tickets.detail.rating.cta': 'Enviar calificación',
  'tickets.detail.rating.sending': 'Enviando...',
  'tickets.detail.rating.error': 'No se pudo enviar la calificación.',
  'tickets.detail.rating.notAllowed': 'Solo podés calificar tickets resueltos o rechazados.',
  'tickets.detail.rating.alreadyRated': 'Ya calificaste este ticket.',
  'tickets.misTickets.soporte': 'Consulta general',

  // --- Ticket messages (US-60) ---
  'ticketMessages.title': 'Chat con soporte',
  'ticketMessages.empty': 'Todavía no hay mensajes.',
  'ticketMessages.infoRequest': 'Solicitud de información del moderador',
  'admin.chat.canalConductor': 'Canal conductor',
  'admin.chat.canalRentador': 'Canal rentador',
  'ticketMessages.error.load': 'No se pudieron cargar los mensajes.',
  'ticketMessages.error.send': 'No se pudo enviar el mensaje.',
  'ticketMessages.error.notAllowed': 'El ticket está cerrado.',

  // --- Disputas (US-64) ---
  'dispute.status.escalated': 'Escalada',
  'dispute.status.awaiting_info': 'Esperando información',
  'dispute.status.ruled': 'Resuelta',
  'dispute.status.appealed': 'Apelada',
  'dispute.status.closed': 'Cerrada',
  'dispute.section.title': 'Disputa',
  'dispute.section.noDispute': 'No hay disputa activa.',
  'dispute.appeal.cta': 'Apelar resolución',
  'dispute.appeal.sending': 'Apelando...',
  'dispute.appeal.success': 'Apelación enviada.',
  'dispute.appeal.error': 'No se pudo apelar.',
  'dispute.appeal.limitReached': 'Ya agotaste el límite de apelaciones.',
  'dispute.appeal.reasonLabel': 'Motivo de la apelación',
  'dispute.appeal.reasonPlaceholder': 'Explicá por qué no estás de acuerdo con la resolución.',
  'dispute.verdict.responsible': 'Responsable',
  'dispute.verdict.penalty': 'Penalidad',

  // --- Soporte contact card (reemplazando strings hardcodeados) ---
  'soporte.contact.chatValue': 'Respuesta inmediata',
  'soporte.contact.emailValue': 'hola@rocketlease.com',
  'soporte.contact.hoursLabel': 'Horario de atención',

  // --- Chat de tickets ---
  'ticketMessages.inputPlaceholder': 'Escribí un mensaje...',
  'ticketMessages.sendAriaLabel': 'Enviar',

  // --- Soporte contactar ---
  'soporte.contactar.cta': 'Reportar un problema',
  'soporte.contactar.asuntoDefault': 'Consulta general',

  // --- Ticket detail extras ---
  'tickets.detail.tiempoEstimado': 'Tiempo estimado de respuesta: menos de 2 horas',

  // --- Admin ---
  'admin.login.tab': 'Admin',
  'admin.login.notAdmin': 'Esta cuenta no tiene permisos de administrador.',
  'admin.tickets.title': 'Cola de tickets',
  'admin.tickets.empty': 'No hay tickets pendientes.',
  'admin.tickets.error': 'No se pudieron cargar los tickets.',
  'admin.tickets.statusFilter.all': 'Todos',
  'admin.tickets.col.asunto': 'Asunto',
  'admin.tickets.col.tipo': 'Tipo',
  'admin.tickets.col.estado': 'Estado',
  'admin.tickets.col.fecha': 'Fecha',
  'admin.tickets.badge.open': 'Abierto',
  'admin.tickets.badge.under_review': 'En revisión',
  'admin.tickets.badge.resolved': 'Resuelto',
  'admin.tickets.badge.closed': 'Cerrado',
  'admin.tickets.markUnderReview': 'Marcar en revisión',
  'admin.tickets.resolveIncident': 'Resolver incidente',
  'admin.resolve.title': 'Resolución del incidente',
  'admin.resolve.close.label': 'Cerrar sin impacto',
  'admin.resolve.close.description': 'El ticket se cierra sin penalizar a ningún usuario.',
  'admin.resolve.fault.label': 'Emitir fallo',
  'admin.resolve.fault.primaryParty': 'Usuario principal',
  'admin.resolve.fault.economic.label': 'Impacto económico (opcional)',
  'admin.resolve.fault.economic.type.absolute': 'Monto fijo (ARS)',
  'admin.resolve.fault.economic.type.percentage': '% del precio de la reserva',
  'admin.resolve.fault.economic.amountHint': 'Positivo = acreditar · Negativo = descontar',
  'admin.resolve.fault.reputation.label': 'Penalización de reputación (opcional)',
  'admin.resolve.fault.reputation.deltaLabel': 'Deducción de score (0–5)',
  'admin.resolve.fault.counterpart.label': 'Contraparte',
  'admin.resolve.fault.mirrorEconomic': 'Espejo económico',
  'admin.resolve.fault.mirrorEconomic.empty': 'Cargá primero un monto fijo en el usuario principal para reflejarlo.',
  'admin.resolve.fault.error.noImpact': 'Configurá al menos un tipo de impacto para el usuario principal.',
  'admin.resolve.fault.error.amount': 'Ingresá un monto distinto de cero.',
  'admin.resolve.fault.error.percentage': 'El porcentaje debe estar entre 1 y 100.',
  'admin.resolve.fault.error.score': 'La deducción de score debe estar entre 0.1 y 5.',
  'admin.resolve.fault.summary.title': 'Resumen del fallo',
  'admin.resolve.fault.summary.suspensionHint': 'A las 3 penalizaciones de reputación, el usuario queda suspendido en ese rol.',
  'admin.resolve.confirm': 'Confirmar resolución',
  'admin.resolve.success.close': 'Ticket cerrado sin impacto.',
  'admin.resolve.success.fault': 'Fallo emitido. Los impactos fueron aplicados.',
  'admin.resolve.error': 'No se pudo resolver el incidente.',
  // --- Crear reseña (US-38) ---
  'resenas.create.title': 'Dejar reseña',
  'resenas.create.targetType.label': '¿Sobre qué querés opinar?',
  'resenas.create.targetType.vehicle': 'Sobre el vehículo',
  'resenas.create.targetType.rentador': 'Sobre el rentador',
  'resenas.create.targetType.conductor': 'Sobre el conductor',
  'resenas.create.rating.label': 'Calificación',
  'resenas.create.comment.label': 'Comentario',
  'resenas.create.comment.placeholder': 'Contá tu experiencia...',
  'resenas.create.cta': 'Enviar reseña',
  'resenas.create.sending': 'Enviando...',
  'resenas.create.success': 'Reseña publicada.',

  // --- Sección de reseñas en detalle (US-38) ---
  'reservas.detail.review.own': 'Tu reseña',
  'reservas.detail.review.counterparty': 'Reseña de {name}',
  'reservas.detail.review.cta': 'Dejar reseña',
  'reservas.detail.review.alreadyReviewed': 'Ya reseñaste',

  // --- Lealtad (US-63) ---
  'lealtad.title': 'Mi Lealtad',
  'lealtad.currentLevel': 'Nivel actual',
  'lealtad.pendingXp': 'Tenés {xp} XP sin acreditar de {count} reservas. Dejá tus reseñas para acreditarlos.',
  'lealtad.xpProgress': '{current} / {next} XP',
  'lealtad.noNextLevel': 'Nivel máximo alcanzado',
  'lealtad.levelUp.title': '¡Felicitaciones!',
  'lealtad.levelUp.subtitle': 'Alcanzaste el nivel {level}',
  'lealtad.levelUp.benefits': 'Beneficios desbloqueados',
  'lealtad.levelUp.cta': 'Ver beneficios',
  'lealtad.levelUp.close': 'Cerrar',
  'lealtad.benefits.title': 'Beneficios de {level}',
  'lealtad.transactions': 'Historial de experiencia',
  'lealtad.transaction.pending': 'Pendiente de acreditación',
  'lealtad.transaction.claimed': 'Acreditada',
  'lealtad.pendingCount': '{count} reservas pendientes de reseña',
  'lealtad.leaveReview': 'Dejar reseña',
  'vehiculos.levelDiscount': '{pct}% OFF por nivel',
  'vehiculos.levelDiscountTooltip': 'Descuento exclusivo por tu nivel {level}',

  // --- Reputación ---
  'reputation.badge.conductor_destacado': 'Conductor Destacado',
  'reputation.banner.suspended': 'Tu cuenta ha sido suspendida por múltiples reportes. Contactá a soporte.',
  'reputation.banner.warning': 'Tu cuenta tiene reportes recientes. Evitá nuevas penalizaciones para no ser suspendido.',
  'reputation.banner.lowScore': 'Tu reputación está baja. Tus vehículos tienen menor visibilidad en las búsquedas.',
} as const

export type I18nKey = keyof typeof es

export function t(key: I18nKey): string {
  return es[key]
}

