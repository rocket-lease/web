...
export function SearchOverlay({ open, onOpenChange, initial, onSearch }: SearchOverlayProps) {
  ...
  // pill se haga temprano en la curva, mientras el slot todavía está
  // shrink-ándose.
  useEffect(() => {
    if (step === prevStep) return
    const id = setTimeout(() => setPrevStep(step), 200)
  ...
  return (
    <div
      ...
      className={cn(
        ...
        expanded
          ? 'h-[calc(100dvh-3rem)] -mx-3 -mb-3 border-x-0 rounded-t-3xl rounded-b-none touch-none'
