...
import { useMyDriverLicenseVerification } from '../hooks/useMyDriverLicenseVerification'
...
export function DriverLicenseVerificationPage() {
  const navigate = useNavigate()
  const { verification, isLoading, isSubmitting, submitVerification, refetch } = useMyDriverLicenseVerification()
  const [frontLicense, setFrontLicense] = useState<SelectedDocument | null>(null)
  const [selfie, setSelfie] = useState<SelectedDocument | null>(null)
  const frontRef = useRef<HTMLInputElement | null>(null)
  const selfieRef = useRef<HTMLInputElement | null>(null)

  const previewUrls = useMemo(
    () => [frontLicense?.previewUrl, selfie?.previewUrl].filter(Boolean) as string[],
    [frontLicense?.previewUrl, selfie?.previewUrl],
  )
  ...
  const handlePick = (setter: (value: SelectedDocument | null) => void, file?: File) => {
    if (!file) return
  ...
  const handleSubmit = async () => {
    if (!frontLicense || !selfie) return

    try {
      await submitVerification({
        frontLicense: frontLicense.file,
        selfie: selfie.file,
      })

      toast.success(t('licencia.toast.submitted'))
      await refetch()
    } catch {
        ...
        <div className="flex flex-1 flex-col items-center justify-center px-5 py-10 text-center">
          ...
          <div className="mt-5 max-w-sm space-y-2">
            <h1 className="text-2xl font-bold text-text-primary">
              {t('licencia.status.verified')}
            </h1>
            <p className="text-sm leading-relaxed text-text-muted">
              {t('licencia.verified.copy')}
            </p>
          </div>
          <Button className="mt-8 w-full max-w-sm" onClick={() => navigate({ to: '/perfil' })}>
            {t('licencia.verified.cta')}
          </Button>
        </div>
      </div>
    )
          ...
          <div className="rounded-3xl border border-warning/20 bg-linear-to-b from-warning-bg to-surface-1 p-5">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <Badge variant="warning">{t('licencia.status.pending')}</Badge>
                <h1 className="mt-3 text-2xl font-bold text-text-primary">
                  {t('licencia.pending.title')}
                </h1>
                <p className="mt-2 text-sm leading-relaxed text-text-secondary">
                  {t('licencia.pending.copy')}
                </p>
              </div>
            </div>
          </div>
          <Button className="mt-6 w-full" variant="secondary" onClick={() => navigate({ to: '/perfil' })}>
            {t('licencia.pending.exit')}
          </Button>
        </div>
      </div>
    )
      ...
      <div className="px-4 py-5">
        <div className="rounded-3xl border border-white/8 bg-linear-to-b from-surface-1 to-surface-0 p-5 shadow-lg shadow-black/10">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <Badge variant={status === 'rejected' ? 'danger' : 'secondary'}>
                {t(`licencia.status.${status}` as const)}
              </Badge>
              <h1 className="mt-3 text-2xl font-bold text-text-primary">
                {t('licencia.form.title')}
              </h1>
              <p className="mt-2 text-sm leading-relaxed text-text-secondary">
                {status === 'rejected' ? t('licencia.rejected.copy') : t('licencia.form.copy')}
              </p>
            </div>
          </div>
          ...
          {status === 'rejected' && verification.rejectionReason && (
            <div className="mt-5 rounded-2xl border border-danger/20 bg-danger-bg p-4 text-sm text-danger">
              <div className="flex items-center gap-2 font-semibold">
                ...
                {t('licencia.rejected.reasonTitle')}
              </div>
          ...
          <div className="mt-5 space-y-4">
            <UploadField
              label={t('licencia.form.front')}
              helper={t('licencia.form.frontHelper')}
              inputRef={frontRef}
              preview={frontLicense?.previewUrl}
              fileName={frontLicense?.file.name}
              onPick={(file) => handlePick(setFrontLicense, file)}
            />
            <UploadField
              label={t('licencia.form.selfie')}
              helper={t('licencia.form.selfieHelper')}
              inputRef={selfieRef}
              preview={selfie?.previewUrl}
              fileName={selfie?.file.name}
              onPick={(file) => handlePick(setSelfie, file)}
            />
          </div>
          ...
          <div className="mt-5 rounded-2xl border border-info/15 bg-info-bg p-4 text-sm text-text-secondary">
            <div className="flex items-center gap-2 font-semibold text-info">
              <WarningCircle size={18} weight="duotone" />
              {t('licencia.form.noteTitle')}
            </div>
            <p className="mt-2 leading-relaxed">
              {t('licencia.form.noteCopy')}
            </p>
          </div>
          ...
          <div className="mt-5 flex flex-col gap-3">
            <Button
              ...
              disabled={isSubmitting || !frontLicense || !selfie}
              className="w-full"
            >
              {isSubmitting ? (
                <span className="inline-flex items-center gap-2">
                  <ArrowClockwise size={18} className="animate-spin" />
                  {t('licencia.form.submitting')}
                </span>
              ) : (
                <span className="inline-flex items-center gap-2">
                  <CloudArrowUp size={18} weight="duotone" />
                  {status === 'rejected' ? t('licencia.form.retry') : t('licencia.form.submit')}
                </span>
              )}
            </Button>

            <Button variant="secondary" size="lg" className="w-full" onClick={() => navigate({ to: '/perfil' })}>
              {t('licencia.form.exit')}
            </Button>
          </div>
          ...
          <div className="mt-6 flex items-start gap-3 rounded-2xl border border-white/8 bg-surface-2 p-4 text-sm text-text-secondary">
            ...
            <p>{t('licencia.form.acceptedCopy')}</p>
          </div>
        </div>
      </div>
    </div>
      ...
      <div className="mt-4 flex items-center gap-3 rounded-xl border border-dashed border-white/10 bg-surface-0 p-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-medium text-text-primary">
            {fileName ?? t('licencia.form.noFile')}
          </p>
          <p className="text-xs text-text-muted">{t('licencia.form.fileHint')}</p>
        </div>
        {preview && (
