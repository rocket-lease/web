...
export function IdentityVerificationPage() {
  ...
  const { verification, isLoading, isSubmitting, submitVerification, refetch } = useMyIdentityVerification()
  const [frontDni, setFrontDni] = useState<SelectedDocument | null>(null)
  const [backDni, setBackDni] = useState<SelectedDocument | null>(null)
  ...
  const previewUrls = useMemo(
    () => [frontDni?.previewUrl, backDni?.previewUrl, selfie?.previewUrl].filter(Boolean) as string[],
    [backDni?.previewUrl, frontDni?.previewUrl, selfie?.previewUrl],
  )
  ...
  const handleSubmit = async () => {
    if (!frontDni || !backDni || !selfie) return

    try {
      await submitVerification({
        frontDni: frontDni.file,
        backDni: backDni.file,
        selfie: selfie.file,
      })
        ...
        <div className="flex flex-1 flex-col items-center justify-center px-5 py-10 text-center">
          ...
          <div className="mt-5 max-w-sm space-y-2">
            <h1 className="text-2xl font-bold text-text-primary">
              {t('identidad.status.verified')}
            </h1>
            <p className="text-sm leading-relaxed text-text-muted">
              {t('identidad.verified.copy')}
            </p>
          </div>
          <Button className="mt-8 w-full max-w-sm" onClick={() => navigate({ to: '/perfil' })}>
            {t('identidad.verified.cta')}
          </Button>
        </div>
      </div>
    )
          ...
          <div className="rounded-3xl border border-warning/20 bg-linear-to-b from-warning-bg to-surface-1 p-5">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <Badge variant="warning">{t('identidad.status.pending')}</Badge>
                <h1 className="mt-3 text-2xl font-bold text-text-primary">
                  {t('identidad.pending.title')}
                </h1>
                <p className="mt-2 text-sm leading-relaxed text-text-secondary">
                  {t('identidad.pending.copy')}
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="px-4 py-5">
        <div className="rounded-3xl border border-white/8 bg-linear-to-b from-surface-1 to-surface-0 p-5 shadow-lg shadow-black/10">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <Badge variant={status === 'rejected' ? 'danger' : 'secondary'}>
                {t(`identidad.status.${status}` as const)}
              </Badge>
              <h1 className="mt-3 text-2xl font-bold text-text-primary">
                {t('identidad.form.title')}
              </h1>
              <p className="mt-2 text-sm leading-relaxed text-text-secondary">
                {status === 'rejected' ? t('identidad.rejected.copy') : t('identidad.form.copy')}
              </p>
            </div>
          </div>
          ...
          {status === 'rejected' && verification.rejectionReason && (
            <div className="mt-5 rounded-2xl border border-danger/20 bg-danger-bg p-4 text-sm text-danger">
              <div className="flex items-center gap-2 font-semibold">
                ...
                {t('identidad.rejected.reasonTitle')}
              </div>
          ...
          <div className="mt-5 space-y-4">
            <UploadField
              label={t('identidad.form.front')}
              helper={t('identidad.form.frontHelper')}
              inputRef={frontRef}
              preview={frontDni?.previewUrl}
              fileName={frontDni?.file.name}
              onPick={(file) => handlePick(setFrontDni, file)}
            />
            <UploadField
              label={t('identidad.form.back')}
              helper={t('identidad.form.backHelper')}
              inputRef={backRef}
              preview={backDni?.previewUrl}
              fileName={backDni?.file.name}
              onPick={(file) => handlePick(setBackDni, file)}
            />
            <UploadField
              label={t('identidad.form.selfie')}
              helper={t('identidad.form.selfieHelper')}
              inputRef={selfieRef}
          ...
          <div className="mt-5 flex flex-col gap-3">
            <Button
              ...
              disabled={isSubmitting || !frontDni || !backDni || !selfie}
              className="w-full"
            >
              {isSubmitting ? (
                <span className="inline-flex items-center gap-2">
                  <ArrowClockwise size={18} className="animate-spin" />
                  {t('identidad.form.submitting')}
                </span>
              ) : (
                <span className="inline-flex items-center gap-2">
          ...
          <div className="mt-6 flex items-start gap-3 rounded-2xl border border-white/8 bg-surface-2 p-4 text-sm text-text-secondary">
            ...
            <p>{t('identidad.form.acceptedCopy')}</p>
          </div>
        </div>
      </div>
    </div>
